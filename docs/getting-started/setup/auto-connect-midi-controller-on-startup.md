...
- AetherSDR debe haberse compilado con soporte MIDI (`Settings > MIDI Mapping...` debe aparecer en el menú Settings).
...
AetherSDR guarda tanto `MidiPort` como `MidiAutoConnect` de inmediato. En el próximo inicio, el puerto se reabre automáticamente sin ninguna acción adicional.
...
| **Refresh** | Botón | Vuelve a examinar los puertos MIDI disponibles | — |
...
- **La lista de puertos aparece vacía después de conectar el controlador** — Haga clic en **Refresh** para volver a examinar los puertos. Si el puerto sigue sin aparecer, verifique que el sistema operativo reconoce el dispositivo.
- **La conexión automática no funciona en el siguiente inicio** — Confirme que hizo clic en **Connect** y vio un estado de conexión exitosa antes de activar **Auto-connect on startup**. El ajuste guarda el nombre del puerto abierto más recientemente; si el nombre del dispositivo cambió (por ejemplo, al usar un puerto USB diferente en algunos sistemas), seleccione el puerto correcto manualmente, conéctese de nuevo y vuelva a activar **Auto-connect on startup**.
