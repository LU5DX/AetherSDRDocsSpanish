...
- El applet de Antenna Genius permanece oculto hasta que se descubre un dispositivo o se conecta manualmente. Si no ve el botón de bandeja del AG en la barra lateral derecha, es lo esperado — aparecerá tras una conexión exitosa.
...
7. Observe la etiqueta de estado debajo del combo de dispositivos. Una conexión exitosa muestra `Connected — <name> v<version>`. Una conexión no alcanzable o rechazada muestra `Error: <msg>`.
...
- Si el combo de dispositivos contiene un dispositivo descubierto, al hacer clic en Connect se conecta a ese dispositivo, no a la dirección de **Manual IP**. Borre o ignore la selección del combo si desea que la dirección de **Manual IP** tenga efecto mediante el botón Connect. Al presionar Enter en el campo **Manual IP** siempre se utiliza la dirección escrita, independientemente del estado del combo.
- El Puerto B se oculta automáticamente si el Antenna Genius conectado reporta solo un puerto de radio.
- La conexión automática al descubrir dispositivos se aplica únicamente a dispositivos Antenna Genius. Si se descubre un ShackSwitch en la misma red, este no se conecta automáticamente desde este applet; es gestionado por el applet de ShackSwitch.
...
- **Un dispositivo descubierto no se conecta automáticamente** — Si el primer dispositivo descubierto en la LAN es un ShackSwitch, el applet de Antenna Genius no se conectará automáticamente a él. El applet de ShackSwitch gestiona ese dispositivo. Verifique si hay un dispositivo Antenna Genius independiente en su red.
