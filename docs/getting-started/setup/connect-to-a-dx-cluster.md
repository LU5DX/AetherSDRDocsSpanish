# Conectarse a un cluster DX

El diálogo SpotHub de AetherSDR le permite conectarse a un cluster DX por telnet, al Reverse Beacon Network, a WSJT-X, a SpotCollector, a POTA y a FreeDV, y configurar cómo se muestran los spots entrantes como superposiciones en el panadapter.

## Antes de comenzar

- Conozca el nombre de host (o dirección IP) y el puerto telnet de su cluster DX elegido (por ejemplo, `dxc.k0xm.net` en el puerto `7373`).
...
3. En el campo **Port:**, configure el puerto telnet (1–65535). Esto se guarda en `ClusterPort`.
...
### Configurar comandos de inicio para el cluster DX

Puede configurar una lista de comandos que se envían automáticamente después de cada inicio de sesión en el cluster DX (por ejemplo, `SET/NAME`, `SET/QTH`, `ACCEPT/SPOT`).
...
|---|---|---|
| **Server:** | Nombre de host o dirección IP del servidor telnet del cluster DX. | `ClusterHost` |
| **Port:** | Puerto telnet. Rango válido: 1–65535. | `ClusterPort` |
| **Callsign:** | Indicativo de inicio de sesión enviado al cluster al conectarse. | `ClusterCallsign` |
| **Connect / Disconnect** | Alterna la conexión telnet. La etiqueta muestra la acción actual. | — |
| **Auto-connect on startup** | Se conecta al cluster automáticamente al iniciar AetherSDR. | `ClusterAutoConnect` |
...
| **Cluster Console** | Pantalla de solo lectura del tráfico telnet sin procesar del cluster. | — |
...
3. En el campo **Port:**, configure el puerto telnet del RBN (1–65535). Esto se guarda en `RbnPort`.
4. En el campo **Callsign:**, escriba su indicativo de inicio de sesión. Esto se guarda en `RbnCallsign`.
5. Configure el **Rate Limit:** para limitar la cantidad de spots del RBN procesados por segundo. Esto se guarda en `RbnRateLimit`.
...
1. Haga clic en **Startup Commands…**.
2. En el diálogo, introduzca un comando por línea.
3. Haga clic en **OK** para guardar. Los comandos se almacenan en la configuración `RbnStartupCommands` y el cliente del RBN los reproduce después de cada inicio de sesión exitoso.
...
| **Connect / Disconnect** | Alterna la conexión al RBN. | — |
| **Auto-connect on startup** | Inicia el RBN automáticamente al arrancar. | `RbnAutoConnect` |
...
| **Spot Color:** | Selector de color para los spots del RBN. | `RbnSpotColor` |
...
3. En el campo **Port:**, configure el puerto UDP. Esto se guarda en `WsjtxPort`.
4. Haga clic en **Start** para comenzar a escuchar. El estado cambia a **Listening**.
5. Para iniciar automáticamente al arrancar, active **Auto-start on startup**. Esto se guarda en `WsjtxAutoStart`.
...
### Personalizar colores de los spots de WSJT-X
...
| **Auto-start on startup** | Inicia el listener automáticamente al arrancar. | `WsjtxAutoStart` |
...
2. En el campo **UDP Port:**, configure el puerto en el que SpotCollector transmite. Esto se guarda en `SpotCollectorPort`.
3. Haga clic en **Start** para comenzar a escuchar. El estado cambia a **Listening**.
4. Para iniciar automáticamente al arrancar, active **Auto-start on startup**. Esto se guarda en `SpotCollectorAutoStart`.
...
| **Start / Stop** | Inicia o detiene el listener UDP. | — |
| **Auto-start on startup** | Inicia el listener automáticamente al arrancar. | `SpotCollectorAutoStart` |
...
2. Configure **Poll Interval:** con los segundos entre consultas. Esto se guarda en `PotaPollInterval`.
3. Haga clic en **Start** para comenzar a consultar. El estado cambia a **Polling**.
4. Para iniciar automáticamente al arrancar, active **Auto-start on startup**. Esto se guarda en `PotaAutoStart`.

### Función de cada control en la pestaña POTA

| Control | Descripción | Clave de configuración |
|---|---|---|
| **Server:** | Endpoint fijo: `api.pota.app` (consulta HTTP). | — |
| **Poll Interval:** | Segundos entre consultas a POTA. | `PotaPollInterval` |
| **Start / Stop** | Inicia o detiene la consulta a POTA. | — |
| **Auto-start on startup** | Inicia POTA automáticamente al arrancar. | `PotaAutoStart` |
...
3. Para iniciar automáticamente al arrancar, active **Auto-start on startup**. Esto se guarda en `FreeDvAutoStart`.
...
| **Auto-start on startup** | Inicia FreeDV automáticamente al arrancar. | `FreeDvAutoStart` |
...
| Control | Descripción | Clave de configuración |
|---|---|---|
| **Bands:** | Casillas de verificación por banda para alternar la visibilidad en la tabla. | — |
...
### Configurar la apariencia de las superposiciones de spots

La pestaña **Display** controla cómo se visualizan los spots en el panadapter.

#### Alternativas maestras

| Control | Descripción | Clave de configuración |
|---|---|---|
| **Spots:** | Alternativa maestra para la superposición de spots DX en el panadapter. Predeterminado: Habilitado. | `IsSpotsEnabled` |
...
| **Auto:** | Cambia automáticamente el modo del slice al hacer clic en un spot que incluye información de modo (p. ej., CW, FT8, RTTY). Predeterminado: Habilitado. La clave de configuración cambió de `SpotsAutoMode` a `SpotAutoSwitchMode` en v26.5.1. El valor predeterminado cambió de Deshabilitado a Habilitado en v0.9.5.1. | `SpotAutoSwitchMode` |

#### Alternativas de Signal History
