...
Esta página explica cómo seleccionar y conectar un controlador MIDI en AetherSDR para que los potenciómetros, faders y botones físicos del dispositivo estén disponibles para asignaciones de parámetros.
...
1. Vaya a `Settings > MIDI Mapping...`. Se abre el cuadro de diálogo **MIDI Controller Mapping**.
2. En la sección **MIDI Device**, abra la lista desplegable **Port:** y seleccione su controlador.
...
4. Haga clic en **Connect**. AetherSDR abre el puerto seleccionado. El área de estado del puerto cambia a **Connected:** seguido del nombre del dispositivo, y el botón **Connect** cambia a **Disconnect**.
5. Gire un potenciómetro o presione un botón en el controlador. El indicador de actividad junto al estado del puerto debe mostrar el mensaje MIDI más reciente recibido (por ejemplo, `Ch 1 CC #7 = 64`). Esto confirma que el dispositivo está enviando datos.
6. Para que AetherSDR reabra este puerto cada vez que se inicie, marque **Auto-connect on startup**.
...
| **Auto-connect on startup** | Casilla de verificación | Cuando está marcada, AetherSDR reabre el último puerto MIDI conectado al iniciarse. | `MidiAutoConnect` |
| **Category** | Desplegable | Filtra el cuadro combinado **Parameter** para mostrar solo los parámetros de la categoría seleccionada. Categorías disponibles: All, RX, TX, Phone/CW, EQ, Global, Mode, Band, Filter, Slice, Display, Frequency. | — |
| **Parameter** | Desplegable | Elige el parámetro a asignar mediante MIDI. Cuando **Category** está en "Phone/CW", hay tres acciones momentáneas (Gate) disponibles: "Trigger straight key" (id: `cwkey`), "Trigger CW Left Paddle" (id: `cwdit`), "Trigger CW Right Paddle" (id: `cwdah`). Los IDs con puntos heredados (`cw.key`, `cw.dit`, `cw.dah`) se migran automáticamente al leerlos. | — |
...
- **`Settings > MIDI Mapping...` no está en el menú** — Su compilación de AetherSDR fue compilada sin soporte MIDI. Obtenga una compilación que incluya la función `HAVE_MIDI`.
...
- [Descripción general de asignación MIDI](../../features/midi-mapping/overview.md)
- [Conectar automáticamente el controlador MIDI al iniciar](auto-connect-midi-controller-on-startup.md)
- [Grabar una nueva asignación con el modo Learn](../../features/midi-mapping/record-a-new-binding-with-learn-mode.md)
