...
Esta página le guía a través de la conexión a su FLEX-8600, la sintonización a una frecuencia, la verificación de la antena y los ajustes de potencia, y la realización de un contacto. Siga estos pasos en orden la primera vez que use AetherSDR.
...
1. Cuando no hay ninguna radio conectada, AetherSDR muestra el panel **Connect to a Radio** en la ventana principal. Si ya lo ha cerrado, ábralo a través de `Settings > Connect to Radio...`.
2. El modo predeterminado es **Local**. Si su radio está en la misma LAN, deje esta opción seleccionada.
3. Espere unos segundos para que la lista de **Available radios** se complete mediante el descubrimiento automático. Su FLEX-8600 debería aparecer por su nombre.
...
1. En la barra lateral derecha, haga clic en el botón de la bandeja **RX** para abrir el applet de controles RX (está visible por defecto).
...
3. Busque el cuadro combinado de antena con etiqueta roja (antena TX, por defecto **ANT1**). Haga clic en él y seleccione el puerto de antena al que está conectada su antena transmisora. Los puertos solo de RX no aparecen en esta lista.
...
4. Elija un ancho de filtro haciendo clic en uno de los botones de **Filter width presets**. Para USB, los preajustes disponibles son 1800, 2100, 2400, 2700, 2900 y 3300 Hz. 2700 Hz es un punto de partida común para SSB.
...
2. Arrastre el control deslizante de **RF Power** a su nivel de potencia deseado (por defecto **100**, rango 0–100). Observe el medidor **RF Pwr** durante una transmisión para confirmar la salida real.
...
4. Si desea ejecutar primero el ATU interno, haga clic en **ATU** y espere a que el ciclo se complete. El indicador **Success** se ilumina en verde cuando se encuentra una adaptación.

### 5. Verificar audio y ganancia

1. De vuelta en el applet de controles RX, confirme que el control deslizante de **AF gain** está en un nivel de escucha cómodo (por defecto **70**, rango 0–100).
2. Confirme que el cuadro combinado **AGC mode** está configurado en **Med** (el predeterminado) para SSB. Ajústelo a **Slow** o **Fast** si es necesario.
...
2. Para transmitir usando el pulsado por software, haga clic en **MOX** en el applet de controles TX. El botón se vuelve rojo mientras transmite. Haga clic en **MOX** nuevamente para volver a recepción.
...
- Si la otra estación está ligeramente fuera de frecuencia y no desea mover su VFO, active **RIT** en el applet de controles RX y use el cuadro de selección **RIT offset** (pasos de 10 Hz) para desplazar su frecuencia de recepción sin cambiar la transmisión. Haga clic en **RIT 0** para ponerlo a cero después.
...
- Use los atajos de teclado de **widen** y **narrow** para recorrer los preajustes de filtro por modo. El método `stepFilterWidth(direction)` recorre la lista de preajustes por modo, por lo que la geometría del borde del filtro siempre es correcta para el modo activo (USB, LSB, CW, AM, DIGL, etc.). Por ejemplo, al presionar un atajo de widen en modo CW, recorre los preajustes de 50, 100, 250, 400 Hz, mientras que en USB recorre los preajustes de 1800, 2100, 2400, 2700, 2900, 3300 Hz.
- Haga clic una vez en el botón de silencio (🔊) para silenciar o reactivar el sonido de este slice. Haga doble clic para silenciar o reactivar todos los slices propietarios a la vez.
- Cambiar a RTTY o modos digitales (DIGU, DIGL) desactiva automáticamente el squelch, que de otro modo eliminaría los caracteres FSK e interrumpiría la decodificación.
...
- **MOX se activa pero el medidor RF Pwr lee cero** — Confirme que la antena TX correcta esté seleccionada en el cuadro combinado de antena TX con etiqueta roja. Confirme que **RF Power** esté por encima de 0 en el applet de controles TX.
- **El medidor SWR lee rojo (superior a 2.5)** — No continúe transmitiendo a máxima potencia. Verifique las conexiones de la antena. Ejecute el **ATU** interno para encontrar una adaptación, o reduzca la potencia hasta que se resuelva el problema.
- **Frequency edit no acepta el valor** — Asegúrese de ingresar un valor en MHz dentro del rango válido (0.001–54.000 MHz para HF, o hasta 50,000 MHz para entradas de XVTR y VHF/UHF). Presione **Escape** para cancelar y restaurar la frecuencia anterior.
- **El menú desplegable de Radio IP address muestra una dirección desactualizada** — La **Source warning label** aparece debajo del cuadro combinado **Advanced: Source path** cuando la NIC guardada previamente ya no es accesible. Seleccione una interfaz de origen válida antes de hacer clic en **Connect by IP (manual)**.
