...
- Localice el panel Band Stack, la franja vertical estrecha de botones de frecuencia de colores que se encuentra junto al panadapter.
...
|---|---|
| Botones de marcador | El clic izquierdo recupera la frecuencia almacenada. El clic derecho abre un menú contextual con la opción **Remove**. |
| **+** | Agrega un nuevo marcador en la frecuencia actual del slice activo. |
| **×** | Borra todos los marcadores del radio conectado. |

La lista de marcadores se almacena de forma persistente bajo `BandStack_<serial>`, donde `<serial>` es el número de serie del FLEX-8600 conectado.
...
- Si la opción "Group by band" está habilitada (mediante el menú ⚙), los marcadores se ordenan por banda en lugar de por orden de inserción. El clic derecho sigue funcionando de la misma manera independientemente de la agrupación actual.
- Para evitar acumular marcadores que rara vez vuelve a consultar, considere habilitar **Auto-expiry** en el menú ⚙. Las opciones disponibles son Off, 5 min, 15 min, 30 min y 60 min.
