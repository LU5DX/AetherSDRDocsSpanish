...
4. En su software de modos digitales (WSJT-X, fldigi o similar), seleccione la fuente PipeWire correspondiente al canal DAX que asignó. El audio ahora llega con aproximadamente 200 ms de latencia en lugar de ~400 ms.

No se requiere configuración adicional. La ruta PipeWire se usa automáticamente en Linux en v0.9.7 y posteriores; no hay un interruptor para cambiar entre la ruta antigua de PulseAudio y la nueva.

## Qué hace cada control

| Control | Valor predeterminado | Rango válido | Clave persistida | Descripción |
|---|---|---|---|---|
| DAX Enable | Off | On / Off | `AutoStartDAX` | Interruptor principal. Inicia todos los flujos RX y TX de DAX. Debe estar activado para que fluya el audio. |
| DAX 1 gain+meter | 0.5 | 0.0 – 1.0 | `DaxRxGain1` | Medidor de nivel y control deslizante de ganancia combinados para el canal DAX 1. Arrastre para ajustar la ganancia de RX. Nombre accesible: "DAX RX 1 gain". |
| DAX 2 gain+meter | 0.5 | 0.0 – 1.0 | `DaxRxGain2` | Medidor de nivel y control deslizante de ganancia combinados para el canal DAX 2. Arrastre para ajustar la ganancia de RX. Nombre accesible: "DAX RX 2 gain". |
| DAX 3 gain+meter | 0.5 | 0.0 – 1.0 | `DaxRxGain3` | Medidor de nivel y control deslizante de ganancia combinados para el canal DAX 3. Arrastre para ajustar la ganancia de RX. Nombre accesible: "DAX RX 3 gain". |
| DAX 4 gain+meter | 0.5 | 0.0 – 1.0 | `DaxRxGain4` | Medidor de nivel y control deslizante de ganancia combinados para el canal DAX 4. Arrastre para ajustar la ganancia de RX. Nombre accesible: "DAX RX 4 gain". |
| TX gain+meter | 0.5 | 0.0 – 1.0 | `DaxTxGain` | Medidor de nivel y control deslizante de ganancia combinados para el flujo TX de DAX. Nombre accesible: "DAX TX gain". |
...
- Para que DAX se inicie automáticamente en cada ejecución, marque `Settings > Autostart DAX with AetherSDR`. Esto establece `AutoStartDAX` en True sin necesidad de hacer clic en Enable manualmente en cada sesión.
- El medidor de nivel utiliza ataque rápido (α = 0.4) y caída lenta (α = 0.08). Una breve ausencia de señal no dejará el medidor en blanco inmediatamente.

## Solución de problemas

- **El botón Enable está atenuado o no responde** — DAX requiere una conexión de radio activa. Conéctese a la FLEX-8600 primero a través de `Settings > Connect to Radio...`, luego haga clic en Enable.
