# Ver qué slice utiliza cada canal DAX en cada momento

El applet DAX Audio muestra un indicador de asignación de slice junto a cada canal DAX, de modo que puede confirmar de un vistazo qué slice está enrutado a cada lugar sin salir de la ventana principal.
...
4. Para ver qué slice está generando el flujo DAX TX, lea la etiqueta de estado en la fila **TX:**. Sigue el mismo formato: `—` o `Slice A` hasta `Slice H`.
...
- Los indicadores se actualizan en tiempo real. Si cambia la asignación de canal DAX de un slice en el radio o en otra parte de la interfaz, el applet refleja el cambio de inmediato sin necesidad de actualización manual.
