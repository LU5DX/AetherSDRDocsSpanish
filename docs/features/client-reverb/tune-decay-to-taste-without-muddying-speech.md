# Ajuste la caída al gusto sin empañar la voz

El control **Decay** determina cuánto tiempo persiste la cola de reverberación después de cada sílaba. Un valor demasiado alto desdibuja el habla; esta página muestra cómo encontrar un valor que añada presencia sin perjudicar la inteligibilidad.
...
1. Ubique el control **Decay**. Muestra un valor en el formato `X.XX s`.
...
4. Retroceda ligeramente hasta que las sílabas ya no se mezclen entre sí. Para la mayoría del trabajo de voz, los valores en el rango de `0.5 s` a `1.5 s` mantienen el habla clara.
5. Si la cola aún suena turbia, aumente **Damp** para atenuar la energía de alta frecuencia en la cola, lo que a menudo reduce la percepción de emborronamiento sin acortar más el Decay.
6. Verifique que **Mix** no esté demasiado alto. Un Mix de `10 %` a `15 %` es típico para voz; el exceso de señal húmeda amplifica el efecto de cualquier valor de Decay.
...
| Visualización de reverberación | ReverbVizBox: visualización en vivo que muestra el paquete de señal seca (cian), las reflexiones de primer orden (amarillo) y la cola reverberante (magenta). Los cinco valores de los controles alimentan la representación, por lo que la visualización sigue las ediciones de los controles en tiempo real. 90 px de alto. | Reemplaza el widget de curva utilizado por otros applets DSP. El algoritmo de representación coincide con StripReverbPanel::GridBox. |

## Visualización en vivo

El panel de applets y el editor flotante "Aetherial FreeVerb — TX" muestran un diagrama de forma de onda compacto (90 px de alto) que se actualiza en tiempo real a medida que mueve cualquier control. El diagrama utiliza tres capas codificadas por colores:
...
## Edición directa de valores de los controles
...
2. Escriba el valor deseado. Puede incluir unidades o caracteres adicionales; el editor los elimina automáticamente. Por ejemplo, `1.5` o `1.5 s` establecen Decay en `1.50 s`.
3. Presione Enter o haga clic fuera del editor para confirmar el valor. El control se actualiza y el editor vuelve a la apariencia de etiqueta.
4. Presione Escape para cancelar la edición y restaurar el valor anterior.

La edición en línea respeta la configuración regional: en regiones que usan coma como separador decimal (como `1,5`), el editor lo analiza correctamente.

## Los colores de los controles siguen el tema actual

A partir de la versión v26.6.1, los colores de los componentes de los controles se obtienen del tema activo en lugar de estar codificados. El applet "Aetherial FreeVerb — TX" usa el espacio de nombres del contenedor `applet/reverb`, que puede definir colores personalizados de primer plano, fondo y manija del control. Si aplica un tema personalizado que cambia los tokens `color.knob.*`, todos los controles de este applet actualizan su arco de anillo, anillo de fondo, puntero de manija, etiqueta y texto de valor para coincidir. La apariencia predeterminada permanece sin cambios para el tema integrado.
...
- **El habla suena lavada incluso con valores cortos de Decay** — Verifique **Mix**. Si Mix supera el `30 %`, la señal húmeda domina independientemente de la longitud de la cola. Reduzca Mix a `10–15 %` primero, luego reevalúe Decay.
