# Ajuste de Ataque y Liberación para un seguidor de envolvente con sonido natural

Ataque (Attack) y Liberación (Release) determinan la rapidez con que el seguidor de envolvente rastrea los niveles de señal ascendentes y descendentes. Solo tienen efecto cuando Envelope está configurado en un valor distinto de cero. Ajustar estos dos mandos evita que el carácter de válvula se active y desactive de forma antinatural durante los transitorios del habla o el audio.
...
4. Localice el mando Release justo debajo de Attack en la columna derecha.
5. Ajuste Release para definir la rapidez con que el seguidor se recupera después de que los niveles disminuyan. Los valores más bajos devuelven la excitación al reposo más rápido; los valores más altos mantienen el efecto durante más tiempo después de un pico.
...
En la versión v26.5.2.1 y posteriores, todos los mandos del editor Aetherial Tube admiten la entrada directa de texto. Haga clic en el valor numérico mostrado debajo de cualquier mando para activar una superposición de edición QLineEdit en línea que reemplaza el texto del valor pintado. Escriba un número (con o sin unidades) y presione Enter, o haga clic en otro lugar, para confirmar. El valor se limita al rango válido del mando.

- Ingrese "12.5 ms", "−6 dB" o "0.75" — el editor elimina las etiquetas finales automáticamente.
- En configuraciones regionales que usan la coma como separador decimal, "12,5" se analiza correctamente.
- Presione Escape para cancelar la edición y revertir al valor anterior.
- Mientras el editor en línea está enfocado, los eventos de la rueda del ratón se redirigen al mando para el ajuste tradicional.
...
| RN2 | Botón de activación solo en TX (oculto en modo RX). Activa el eliminador de ruido neuronal RNNoise en la entrada del micrófono antes de la cadena DSP. Suprime el ruido de fondo antes de que llegue al compuerta/compresor/saturador. | Ubicado en el StripTubePanel flotante debajo del medidor de nivel de salida, solo en el lado TX. Solo modos de voz — los modos digitales (RADE, DAX, RTTY, FT8, FDV, CW) omiten esta etapa. La configuración se conserva mediante AudioEngine. |

**Attack** — Utiliza mapeo exponencial en todo su rango. Define la rapidez con que el seguidor de envolvente responde a niveles de señal ascendentes cuando Envelope ≠ 0. La etiqueta se muestra como "X.XX ms" por debajo de 10 ms y como "X.X ms" por encima de 10 ms. Clave de configuración: `ClientTubeTxAttackMs` / `ClientTubeRxAttackMs`.

**Release** — Utiliza mapeo exponencial en todo su rango. Define la rapidez con que el seguidor de envolvente se recupera después de que los niveles de señal disminuyen cuando Envelope ≠ 0. La etiqueta se muestra como "X.XX ms" por debajo de 100 ms y como "X.X ms" por encima de 100 ms. Clave de configuración: `ClientTubeTxReleaseMs` / `ClientTubeRxReleaseMs`.

**RN2** — Botón de activación solo en TX ubicado en el StripTubePanel debajo del medidor de nivel de salida. Utiliza la etiqueta `RN2`. Consulte [Reduce background noise with RN2](reduce-background-noise-with-rn2.md) para obtener detalles completos.

## Consejos

- Attack y Release son independientes por lado. Los cambios realizados en el editor TX no afectan al editor RX y viceversa.
- Para un habla con sonido natural en TX, comience con los valores predeterminados (Attack 5.00 ms, Release 35.00 ms) y alargue Release primero. Valores de Release muy cortos pueden producir un carácter de bombeo o parloteo.
...
- La bola de entrada en vivo en la curva de transferencia se mueve en tiempo real. Con Envelope configurado, puede ver el punto de operación desplazándose a medida que cambian los niveles, lo que ayuda a confirmar que Attack y Release se sienten correctos antes de salir al aire.
- Cuando la etapa Tube está desactivada, todo el mosaico de la applet acoplada se atenúa aproximadamente al 55 % de opacidad. Esta señal visual se aplica tanto a los mosaicos TX como RX y facilita la confirmación de un vistazo de que la etapa no está procesando audio. El mosaico vuelve a la opacidad completa tan pronto como la etapa se reactiva.
- Los colores visuales de los mandos (anillo de fondo, arco, puntero, etiqueta y texto del valor) están controlados por el tema en la versión v26.6.1 y posteriores. Respetan la anulación del contenedor por applet (por ejemplo, el color ámbar del primer plano del mando exclusivo de la applet de compresión) recorriendo la jerarquía de widgets hasta `applet/tube`.

## Solución de problemas

- **Ajustar Attack o Release no tiene efecto audible** — Es probable que Envelope esté configurado al 0 %. Configure Envelope primero con un valor positivo o negativo; Attack y Release solo se aplican cuando el seguidor de envolvente está activo.
- **El efecto suena demasiado abrupto o con parloteo** — Release está configurado demasiado bajo. Aumente Release hacia 100 ms o más para suavizar la recuperación.
