...
El saturador dinámico de válvula del lado cliente de AetherSDR añade riqueza armónica moldeando la señal a través de un modelo de válvula con polarización variable. El sistema instancia dos copias independientes: una para la cadena de transmisión ("Aetherial Mic-PreAmp") y una para la cadena de recepción ("Aetherial Dynamic Tube").

## Abrir los controles de la válvula

- **Mosaico de applet acoplado**: Haga clic en "Aetherial Mic-PreAmp" (TX) o "Aetherial Dynamic Tube" (RX) en el dock de applets. El mosaico compacto muestra una curva de transferencia con una bola de entrada en vivo y cinco perillas de ajuste.
...
| Botón | Valor por defecto | Comportamiento |
|---|---|---|
| A | Marcado | Selecciona el modelo de carácter de válvula A. Ámbar cuando está marcado. |
| B | Sin marcar | Selecciona el modelo de carácter de válvula B. Ámbar cuando está marcado. Exclusivo con A y C. |
| C | Sin marcar | Selecciona el modelo de carácter de válvula C. Ámbar cuando está marcado. Exclusivo con A y B. |
...
El editor en línea admite el análisis de decimales según la configuración regional, por lo que valores como "12,5" funcionan en configuraciones regionales con coma decimal. También puede incluir texto de unidad: el analizador elimina automáticamente los caracteres no numéricos.
...
Mapeo lineal. Los valores negativos oscurecen la señal saturada; los valores positivos la aclaran. Fila central del editor, a la izquierda del selector de modelo.
...
Ganancia de compensación/ajuste posterior a la válvula. Columna izquierda del editor.
...
Mapeo lineal. Mezcla de señal seca/húmeda (100 % = señal completamente saturada). Columna izquierda del editor (perilla superior).
...
Mapeo lineal (-1.0 a +1.0). Los valores positivos aumentan la excitación en los transitorios (la válvula se calienta más en los picos fuertes); los valores negativos la reducen, comprimiendo los armónicos dinámicamente. Columna derecha del editor.

### Attack

- **Formato de etiqueta**: `X.XX ms` (por debajo de 10 ms), `X.X ms` (por encima de 10 ms)
- **Valor por defecto**: 5.00 ms
...
El ClientTubeCurveWidget en modo compacto dibuja la curva de transferencia de válvula configurada actualmente con una bola en vivo en el nivel de entrada. El widget de curva utiliza colores que siguen el tema:
...
## RN2 (Reducción de ruido solo en TX)
...
- La alternancia RN2 está oculta en modo RX.
- Solo está disponible en modos de voz. Los modos digitales (RADE, DAX, RTTY, FT8, FDV, CW) omiten esta etapa.
- El ajuste se persiste a través del AudioEngine.
...
- Los cambios realizados en el editor flotante y en el applet acoplado se mantienen sincronizados. Un temporizador de sondeo de 30 Hz mantiene ambas vistas actualizadas, por lo que ajustar cualquier control en un lugar se refleja inmediatamente en el otro.
...
- Cuando el mosaico del applet aparece atenuado, la etapa de válvula está en bypass. Vuelva a habilitarla antes de realizar ajustes; de lo contrario, los controles no tienen efecto en la señal en vivo.
