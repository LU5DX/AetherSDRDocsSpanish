...
El control Output aplica un ajuste de ganancia post-tubo a la señal procesada. Úselo para compensar el aumento o la reducción de nivel que introducen Drive y Bias, de modo que la etapa de tubo no eleve ni baje involuntariamente los niveles en el resto de la cadena.
...
1. Localice el control **Output** en la fila de cinco controles (el cuarto desde la izquierda: Drive, Tone, Bias, **Output**, Mix).
2. Gire **Output** en sentido horario para aumentar el nivel post-tubo, o en sentido antihorario para reducirlo.
3. Suelte el control cuando el nivel de salida coincida con su objetivo. La etiqueta debajo del control se actualiza en tiempo real y muestra el valor actual en dB (por ejemplo, `0.0 dB`).
...
| Control | Valor predeterminado | Rango válido | Clave de ajuste persistido |
|---|---|---|---|
...
El editor flotante incluye un medidor de pico **OUT** (el widget `ClientLevelMeter`) ubicado en el extremo derecho del panel del editor. Muestra el nivel de pico post-saturación con una dinámica de ataque rápido y liberación lenta, y utiliza las siguientes bandas de color:
...
El medidor no está presente en el panel del applet acoplado. Se actualiza continuamente junto con los controles de mando cuando el editor flotante está abierto.
...
- Los cambios realizados en el editor flotante y en el applet acoplado se mantienen sincronizados. Un temporizador de sondeo a 30 Hz mantiene ambas vistas actualizadas, por lo que ajustar Output en una ubicación se refleja inmediatamente en la otra.
