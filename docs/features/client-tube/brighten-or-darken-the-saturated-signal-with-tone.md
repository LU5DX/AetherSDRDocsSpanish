# Aclare u oscurezca la señal saturada con Tone

Use el control Tone para inclinar el carácter espectral de la señal saturada: los valores negativos la oscurecen y la hacen más cálida, los valores positivos la aclaran y la hacen más presente. Tone funciona tanto en el lado TX (Aetherial Mic-PreAmp) como en el lado RX (Aetherial Dynamic Tube) de forma independiente.
...
| RN2       | Alternancia solo TX (oculto en modo RX). Habilita el eliminador de ruido neuronal RNNoise en la entrada de micrófono antes de la cadena DSP. Suprime el ruido de fondo antes de que llegue a puerta/compresor/saturador. | Ubicado en el editor flotante StripTubePanel debajo del medidor de nivel de salida, solo lado TX. Solo modos de voz; los modos digitales (RADE, DAX, RTTY, FT8, FDV, CW) omiten esta etapa. La configuración se conserva mediante AudioEngine. |
...
- El editor de valor en línea admite el análisis de números según la configuración regional. Ingrese `12,5` en una configuración regional de coma decimal, o elimine unidades como `12.5 ms` y el editor analizará la parte numérica.

## Solución de problemas

- **El mando Tone no tiene efecto audible**: es posible que la etapa de tubo esté en bypass. Confirme que la etapa esté activa en el widget CHAIN del lado correspondiente. También verifique que Mix esté por encima del 0 %; una señal completamente seca (Mix al 0 %) atraviesa el modelo de tubo pero no mezcla nada de la salida procesada.
- **La posición del mando no coincide con lo esperado después de recargar**: el valor se guarda automáticamente cada vez que el mando cambia. Si `ClientTubeTxTone` o `ClientTubeRxTone` falta o está dañado en su archivo de configuración, el valor vuelve al valor predeterminado de 0.00.
