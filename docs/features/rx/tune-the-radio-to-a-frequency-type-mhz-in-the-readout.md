...
| **ANT1 (antena RX)**       | Abre un menú con las antenas RX disponibles. Al seleccionar, se establece la antena de recepción del slice.                             | ANT1           |
| **ANT1 (antena TX)**       | Abre un menú con las antenas capaces de TX. Los puertos solo RX (prefijo 'RX') se filtran.                                             | ANT1           |
| **2.7K (ancho de filtro)** | Muestra el ancho de banda actual del filtro (p. ej., 2.7K, 3.3K, 500).                                                                 | 2.7K           |
...
| **Combo de modo**          | Establece el modo de operación del slice. Las opciones varían según la radio y las banderas de compilación. La opción RADE requiere la bandera de compilación HAVE_RADE. | USB            |
...
| **Widget de banda pasante del filtro** | Arrastre los bordes lo/hi para ajustar directamente la banda pasante del filtro.                                                      | —              |
...
| **🔊 / 🔇 (silencio)**      | Silencia o reactiva la salida de audio del slice.                                                                                      | 🔊 (sin silencio) |
| **Ganancia AF**            | Ajusta la ganancia de salida de audio del slice (0–100).                                                                              | 70             |
| **Pan L / R**              | Desplaza el audio del slice entre los canales izquierdo y derecho (0–100). Haga doble clic para restablecer a 50 (centro).              | 50             |
| **SQL**                    | Activa el squelch en el nivel actual del deslizador. Deshabilitado en modos RTTY y digitales (DIGU, DIGL, NT) donde el squelch eliminaría caracteres FSK. | —              |
| **Nivel de squelch**       | Ajusta el umbral de squelch (0–100). Solo tiene efecto cuando SQL está activado. Deshabilitado en modos RTTY y digitales.               | 20             |
...
## Paso de ancho de filtro

Desde v0.9.8, el applet admite un paso de ancho de filtro preciso y correcto para el modo a través de la lista de preajustes por modo. El método `stepFilterWidth()` recorre la lista de preajustes para el modo actual, encuentra el preajuste más cercano al ancho de filtro actual y aplica el siguiente o anterior preajuste en la dirección elegida.

Esto significa:

- Los atajos de ampliar y reducir producen anchos de filtro que coinciden con la lista de preajustes del modo actual (SSB, CW, AM, digital, etc.).
...
Desde v0.9.5.1, los preajustes de ancho de filtro pueden almacenar un valor de ancho de banda simple o un par explícito de borde inferior/superior. Esto coincide con el formato de almacenamiento utilizado por VfoWidget y permite que los preajustes guardados desde VfoWidget se carguen correctamente en el applet de Controles RX.
...
| `lo:hi` | `-1350:1350` | Bordes explícitos de la banda pasante en Hz con respecto a la portadora. Ambos valores deben ser enteros y `hi` debe ser mayor que `lo`. |
...
Normalmente no necesita editar estos valores manualmente. Al hacer clic derecho en un botón de **Preajustes de ancho de filtro** se guarda el ancho de filtro actual en esa ranura utilizando el formato apropiado automáticamente.

## Modo NT

v0.9.3 agrega el modo **NT** al selector de modos. NT se comporta como un modo digital de las siguientes maneras:
...
- **El modo RTTY** ahora se incluye en la lógica de desactivación de squelch junto con DIGU, DIGL y NT. Cuando cambia al modo RTTY, el botón **SQL** y el deslizador de nivel de squelch se deshabilitan, y cualquier squelch activo se desactiva automáticamente. Esto evita que el squelch elimine caracteres FSK y rompa la decodificación.
- **El cambio de modo RADE** se mejora con una mejor lógica de defensa en profundidad. Cuando cambia de RADE a otro modo, el applet ahora lee el modo del slice antes de aplicar el cambio. Esto asegura que la señal `radeActivated` solo se emita para el slice que realmente estaba en modo RADE, evitando señales de desactivación espurias en escenarios como:
  - Reenlace de `setSlice()` de una sola instancia
  - RADE activado externamente (a través del combo VfoWidget, carga de perfil al inicio, o MainWindow::activateRADE)
  - Uso de múltiples paneles donde cambia USB→LSB en un slice que no es RADE

## Consejos

- No necesita escribir ceros a la derecha. `14.2` se interpreta como 14.200 MHz.
- Para mover la frecuencia en pequeños pasos sin volver a escribir, use los botones `<` y `>` junto a **STEP**, o desplace la rueda del ratón sobre la **Etiqueta de frecuencia** después de sintonizar.
- La lista de tamaño de paso cambia cuando cambia de modo. Cambiar el **Combo de modo** restablece los preajustes de paso a valores apropiados para ese modo.
- Los atajos de ampliar y reducir el ancho de filtro recorren la lista de preajustes por modo desde v0.9.8, dando una geometría de filtro correcta para el modo en cada paso.

## Solución de problemas

- **El campo de Edición de frecuencia no aparece cuando hago clic en el indicador** — El slice puede estar bloqueado. Verifique si el icono de bloqueo muestra 🔒. Si es así, haga clic en él para desbloquearlo, luego haga clic en el indicador de frecuencia nuevamente.
...
- **El botón SQL aparece atenuado después de cambiar al modo NT o RTTY** — Esto es normal. NT y RTTY son modos digitales; el squelch se desactiva automáticamente, igual que para DIGU y DIGL.
- **Los botones de pestaña de slice muestran los slices incorrectos después de reconectar** — Este era un problema conocido en versiones anteriores a v0.9.5.1. Actualice a v0.9.5.1 o posterior; la fila
