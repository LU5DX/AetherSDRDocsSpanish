...
El panadapter muestra un espectro FFT en tiempo real y un waterfall para una slice de radio, lo que permite visualizar la actividad de la banda y sintonizar haciendo clic o arrastrando. Cada panadapter también puede mostrar un panel opcional de decodificación CW que lee código Morse del aire directamente en la aplicación.

## Antes de comenzar

- AetherSDR debe estar conectado a una radio FLEX-8600. El panadapter requiere una conexión de radio activa.
- Para la decodificación CW, el enrutamiento de audio de PC hacia AetherSDR debe estar configurado. El panel muestra un recordatorio "(requires PC Audio)" cuando esto no está en orden.
...
|---|---|---|---|
| Título de slice | Indicador | Muestra la slice vinculada a este panadapter. Valores: Slice A – Slice H. | — |
| ⬈ / ↩ (ventana flotante/acoplar) | Botón | Extrae el panadapter a una ventana flotante, o lo vuelve a acoplar. | Oculto en modo de panadapter único. La ventana flotante no tiene marco; arrastre mediante la barra de título integrada en la aplicación, cambie el tamaño mediante el control de tamaño en la esquina inferior derecha. |
| □ (maximizar) | Botón | Maximiza este panadapter para ocupar el área de diseño principal. | Oculto en modo de panadapter único. |
...
| 🔒S (Lock Speed) | Botón de alternancia | Off | On / Off | — | Fija la velocidad del decodificador a la lectura WPM actual. |
...
| Texto de decodificación CW | Campo de texto de solo lectura | — | — | — | Visualización continua del código Morse decodificado, con color según la confianza: verde (mayor confianza), pasando por amarillo y naranja hasta rojo (menor confianza). |

## Consejos

- Los controles deslizantes de tono Lo y Hi limitan el rango de frecuencias en el que busca el decodificador. Reducir este rango alrededor del tono CW esperado disminuye las decodificaciones falsas en una banda concurrida.
- El color del texto decodificado refleja la confianza del decodificador. El texto verde es el más confiable; el texto rojo debe tratarse con precaución. Aumente Sens para suprimir los caracteres rojos y naranja si el ruido está generando salida incorrecta.
