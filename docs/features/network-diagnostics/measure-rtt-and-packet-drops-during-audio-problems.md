...
Utilice el cuadro de diálogo Network Diagnostics para leer el tiempo de ida y vuelta en vivo y los conteos de pérdida de paquetes por categoría mientras ocurren los problemas de audio. Esto le permite distinguir la pérdida de red de otras causas como falta de datos en el búfer o jitter.

## Antes de comenzar

- AetherSDR debe estar ejecutándose. El cuadro de diálogo no requiere una conexión activa al radio, pero el RTT y los contadores de pérdida solo son significativos mientras está conectado.
...
|-----------|-------------|-------|
| `Latency (RTT)` | Tiempo de ida y vuelta actual al radio. Muestra `< 1 ms` cuando es inferior a 1 ms. | |
...
| `Waterfall` (Packet Loss) | Misma métrica para el flujo del waterfall. | |
...
| Logs (pestaña) | Seguimiento en vivo del archivo de registro de AetherSDR, filtrado por casillas de verificación de categoría. Resaltado de sintaxis por nivel de registro y nombre de categoría. | El selector de marco temporal está oculto mientras esta pestaña está activa. |
| Timeframe | Selecciona qué tan atrás muestran el historial los gráficos de series temporales. El valor predeterminado es 5 minutos; las opciones son 1 minuto, 5 minutos, 15 minutos, 1 hora, 1 día y 1 semana. | Se muestra en la esquina superior derecha de la barra de pestañas; oculto cuando la pestaña Logs está activa. |
...
El cuadro de diálogo ahora utiliza un panel de navegación en forma de árbol en el lado izquierdo en lugar de una barra de pestañas. El árbol contiene los siguientes elementos:

- **Overview** — Mismo contenido que la antigua pestaña Overview
- **Details** — Mismo contenido que la antigua pestaña Details
...
El árbol de navegación admite navegación por teclado con las teclas de flecha. El elemento seleccionado está resaltado. El cuadro combinado Timeframe permanece en la esquina superior derecha del cuadro de diálogo, oculto cuando la página Logs está activa.
...
- Una pérdida cero en la sección Packet Loss no descarta el problema. El jitter y la entrega tardía en ráfagas pueden causar cortes de audio sin provocar espacios en los números de secuencia. Si las pérdidas son cero pero el audio aún está dañado, verifique `Underruns (total)`, `Underruns (last sec)`, `Audio Arrival Gap`, `Max Arrival Gap` y `Jitter Estimate` en la sección **Audio Playback**.
- `Max Latency (RTT)` es más útil que el RTT actual para detectar picos transitorios que ya han pasado.
- La pérdida que aparece simultáneamente en todas las categorías de flujo apunta a un problema de ruta de red compartida en lugar de un problema específico de audio.
...
- Use la pestaña **Logs** con los filtros de categoría adecuados para correlacionar eventos de registro sin procesar con las métricas que se muestran en las otras pestañas.
- La configuración del modo sin marco afecta a todos los cuadros de diálogo de AetherSDR que admiten este modo. Si el cuadro de diálogo Network Diagnostics no muestra una barra de título, verifique que `FramelessWindow` esté habilitado en Preferences.
...
- **Todos los contadores de pérdida muestran 0 / 0** — No se han recibido paquetes VITA en esa categoría. Confirme que el radio está conectado y transmitiendo los flujos relevantes.
- **El RTT muestra `< 1 ms` pero el audio está dañado** — La latencia de red no es la causa. Consulte la sección Audio Playback para obtener datos de subejecución y jitter.
...
- [Diagnostique subejecuciones de audio y jitter](../../troubleshooting/networkdiagnostics/diagnose-audio-underruns-and-jitter.md)
- [Verifique las tasas de datos por categoría (audio, FFT, waterfall, meters, DAX)](check-per-category-data-rates-audio-fft-waterfall-meters-dax.md)
- [Descripción general de Network Diagnostics](overview.md)
- [Verifique la IP del radio y la dirección de enlace local](verify-the-radio-s-ip-and-local-bind-address.md)
