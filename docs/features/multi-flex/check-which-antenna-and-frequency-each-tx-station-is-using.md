...
El panel multiFLEX muestra la antena TX y la frecuencia TX de cada estación que comparte el FLEX-8600 en ese momento. Use esta información para confirmar que los demás operadores del radio no interfieren con la frecuencia de operación o la antena que usted tiene previstas.

## Antes de comenzar

- AetherSDR debe estar conectado al radio.
- multiFLEX debe estar habilitado en el radio. Si la tabla Stations está vacía o la función no está activa, consulte [Habilitar multiFLEX en el radio](enable-multiflex-on-the-radio.md).
...
3. Lea la columna **TX ANT** para conocer la antena desde la que transmite esa estación.
4. Lea la columna **TX FREQ (MHz)** para conocer la frecuencia de transmisión de esa estación en MHz, expresada con tres decimales.
...
| Columna LOCAL PTT | Muestra una marca de verificación junto a la estación que actualmente tiene autoridad PTT. |
...
- Los valores de TX ANT y TX FREQ provienen del estado del slice del radio. Si una estación acaba de conectarse, esas columnas pueden mostrar `----` brevemente hasta que el radio reporte los datos del slice.
