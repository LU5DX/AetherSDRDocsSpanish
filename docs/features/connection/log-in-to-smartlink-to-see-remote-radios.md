...
5. Haga clic en `Sign In`. AetherSDR se autentica con SmartLink y, si el proceso es exitoso, llena la lista `Remote radios` con los radios disponibles para su cuenta.
...
| `Remote radios` | Muestra los radios WAN disponibles para su cuenta después de iniciar sesión. La lista tiene una altura de visualización fija; si tiene muchos radios remotos, desplácese dentro de la lista para verlos todos. | — |
| `Connect Remote Radio` | Inicia una conexión WAN al radio seleccionado en la lista `Remote radios`. | — |
| `Sign Out` | Finaliza la sesión actual de SmartLink. | — |

## Consejos

- `SmartLinkEmail` se conserva entre sesiones, por lo que su dirección de correo electrónico aparece completada automáticamente en el siguiente inicio. La contraseña nunca se almacena.
...
- La lista `Remote radios` está dimensionada para mostrar una vista compacta de los radios disponibles. Desplácese dentro de la lista si la entrada que busca no está visible de inmediato.

## Solución de problemas

- **La lista `Remote radios` aparece vacía después de iniciar sesión** — Es posible que el radio remoto esté apagado, no esté registrado en SmartLink o esté asociado a una cuenta de FlexRadio diferente. Confirme que el radio esté encendido y registrado en la misma cuenta con la que inició sesión.
