...
- El slice activo debe estar en modo CW o CWL. Las teclas de función F1–F12 se ignoran en silencio en todos los demás modos.
...
2. Presione la tecla de función (F1 a F12) en su teclado que corresponda a la macro que desea enviar. La macro se dispara inmediatamente.
...
|---|---|---|---|---|
| F1 … F12 (teclado) | Envía la macro almacenada para esa tecla de función. Solo activo cuando el panel CWX está visible y el modo del slice es CW o CWL. | — | — | `CwxMacro_F1` … `CwxMacro_F12` |
...
| QSK | Activa el modo de interrupción completa (QSK). | Off | On / Off | `CwxQsk` |

## Cómo interactúan Send y Live

El comportamiento del botón **Send** cambió en v0.9.2.1. Su acción ahora depende de si el modo **Live** está activo actualmente:
...
- **Live está activado** — Al hacer clic en **Send** primero se desactiva el modo Live y se devuelve el panel a la vista normal de envío. El búfer **no** se retransmite. Esto evita que el texto que ya fue tecleado carácter por carácter en modo Live se envíe una segunda vez.

El botón **Live** ahora es un conmutador. Al hacer clic nuevamente mientras Live está activo, se desactiva el modo Live. Cuando un modelo conectado cambia el estado Live externamente (por ejemplo, desde otro panel o un evento del radio), el botón **Live** se actualiza automáticamente para reflejar el estado actual.

Al hacer clic en **Setup**, siempre se desactiva el modo Live antes de mostrar la vista del editor de macros.

## Consejos

- Los atajos F1–F12 son globales para la aplicación. Solo funcionan cuando el panel CWX está visible. Si cambia a otro panel que también usa F1–F12 (como el panel DVK), los atajos se deshabilitan para evitar ambigüedad.
- Presionar Escape durante el envío de una macro es la forma más rápida de abortar. Debido a que el radio transita entre los estados de transmisión y listo rápidamente durante CW, Escape funciona independientemente del estado actual de transmisión del radio.
...
- **Presionar F1–F12 no hace nada** — Confirme que el slice activo esté en modo CW o CWL y que el panel CWX esté visible. Los atajos están deshabilitados cuando el panel está oculto u otro panel que usa F1–F12 está visible. Si otra aplicación ha capturado las teclas de función, traiga AetherSDR al primer plano.
...
- [Activar QSK de interrupción completa](enable-qsk-full-break-in.md)
