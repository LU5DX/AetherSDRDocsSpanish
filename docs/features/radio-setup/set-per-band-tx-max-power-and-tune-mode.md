...
El diálogo de Configuración de la Radio es la ventana maestra de configuración por radio. Organiza la identificación de la radio, red, GPS, transmisión, teléfono/CW, recepción, nombres de antenas, filtros, transverters, cables USB, periféricos, APD, temas, gestión de certificados SmartLink y configuraciones de puertos serie en múltiples pestañas.
...
| **Station Name** | Campo de texto | Identifica este cliente de AetherSDR ante otras estaciones multiFLEX. Por defecto usa el nombre de host del SO si está vacío. Se almacena en AppSettings. Se envía a la radio como 'client station <name>'. |

Cada campo de información de solo lectura en esta pestaña (Radio SN, Callsign, Options, HW Version, Region, Model) muestra un pequeño icono de copiar al pasar el ratón sobre el valor. Haga clic en el icono para copiar el texto al portapapeles. Un breve mensaje emergente "Copied" confirma la acción.

### Información de licencia

El diálogo muestra los detalles de la licencia de la radio, incluidos el estado de la suscripción, la fecha de vencimiento, el ID de la radio y la versión licenciada.

### Actualización de firmware

1. Haga clic en **Check for Update** para buscar actualizaciones de firmware.
...
| Control | Tipo | Notas |
|---|---|---|
| **Check for Update** | Botón | Busca actualizaciones de firmware disponibles. |
...
| **Network MTU:** | Spin box | 1450 | 576–9000 bytes | Establece el tamaño máximo del paquete UDP VITA-49 de salida en bytes. El valor predeterminado de 1450 es seguro para la mayoría de los túneles VPN/SD-WAN. Almacenado en AppSettings como `NetworkMtu`. |
...
Use esta página para configurar los ajustes de transmisión, incluyendo tiempos, enclavamientos, potencia máxima, modo de sintonía, visualización en waterfall y comportamiento de seguimiento de slice/TX.
...
|---|---|---|---|
| **Max Power:** | Spin box | — | 0-100%. Establece el límite de potencia de TX a nivel de radio. |
| **Tune Mode:** | Combo box | — | Selecciona cómo se comporta el botón de sintonía. |
| **Timings** | Spin box / Campo de texto | — | Tiempos de espera/retardo de TX (ver tabla a continuación). |
...
| **TX Follows Active Slice** | Botón | Falso | TX sigue al slice activo. Mutuamente excluyente con 'Active Slice Follows TX'. Se desactiva automáticamente durante una operación de Split. |
| **Active Slice Follows TX** | Botón | Falso | Cambia el slice activo cuando TX se mueve externamente (ej. WSJT-X o CAT). Mutuamente excluyente con 'TX Follows Active Slice'. |

### Campos de tiempo de TX

| Campo | Etiqueta mostrada | Unidad reportada | Sufijo de comando |
|---|---|---|---|
...
| Control | Tipo | Notas |
|---|---|---|
| **ANT1 / ANT2 / XVTA / XVTB** | Campo de texto | Etiqueta amigable para cada puerto de antena. Los nombres aparecen en el selector de antena del band scope y en el menú desplegable de antena TX. |
...
La pestaña Phone/CW configura los valores predeterminados del micrófono, el keyer CW y RTTY.

### Configuración del keyer CW

| Control | Tipo | Valor predeterminado | Rango válido | Notas |
|---|---|---|---|---|
| **Iambic:** | Botón de alternancia | — | Habilitado / Deshabilitado | Habilita o deshabilita el keyer iambic en la radio. |
| **Iambic Mode: A / B** | Botón | A | A / B | Selecciona el modo iambic Curtis A o B tanto para la radio como para el keyer de software local. Par mutuamente excluyente. |
...
La pestaña RX contiene controles para la calibración manual de desplazamiento de frecuencia y la selección de la fuente de referencia de 10 MHz. Los controles de calibración siempre se muestran independientemente de si hay un GPSDO instalado.
...
| **Freq Offset (ppb):** | Spin box | Desplazamiento de frecuencia manual en partes por billón. |
...
| **Audio Buffer:** | Campo de texto | Predeterminado: 200. Rango válido: 50–1000 ms. Aumenta el búfer de audio para jitter de VPN/SmartLink. |
...
| **Save to:** | Campo de texto | — | Carpeta para las grabaciones guardadas (solo lado del cliente). Por defecto en Documents/AetherSDR/Recordings. |
| **...** | Botón | — | Examina para seleccionar la carpeta de grabación. |
| **Auto-record on TX** | Casilla de verificación | Falso | Graba automáticamente mientras se transmite. |
...
| Control | Tipo | Valor predeterminado | Notas |
|---|---|---|---|
| Deslizadores de nitidez de filtro Voice / CW / Digital | Deslizador | — | Establece la nitidez del filtro (0=menor latencia a 3=más nítido) por modo. El deslizador está deshabilitado cuando Auto está habilitado. |
