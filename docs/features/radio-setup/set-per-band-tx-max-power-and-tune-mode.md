...
| Control | Tipo | Notas |
|---|---|---|
| **Radio SN** | Indicador | Número de serie del chasis (solo lectura). Haga clic en el icono de copia junto al valor para copiarlo al portapapeles. |
...
| **Options** | Indicador | Muestra las opciones de radio licenciadas. Haga clic en el icono de copia para copiar el texto. |
...
| **Callsign** | Campo de texto | Indicativo de la estación. Haga clic en el icono de copia junto al valor para copiarlo al portapapeles. |
| **Station Name** | Campo de texto | Identifica este cliente de AetherSDR ante otras estaciones multiFLEX. Por defecto, usa el nombre de host del SO si está vacío. Almacenado en AppSettings. Enviado a la radio como 'client station <nombre>'. |

### Copiar valores de solo lectura

Cada campo de información de solo lectura en esta pestaña (Radio SN, Callsign, Options, HW Version, Region, Model, IP Address, Mask, MAC Address y campos de Información de Licencia) muestra un pequeño icono de copia al pasar el cursor sobre el valor. Haga clic en el icono para copiar el texto al portapapeles. Una breve ventana emergente "Copied" confirma la acción.

### Información de licencia

El diálogo muestra los detalles de la licencia de la radio, incluido el estado de suscripción, fecha de vencimiento, ID de radio y versión licenciada. Cada campo incluye un botón de copia al portapapeles junto al valor.
...
3. Haga clic en **Select Installer...**.
   ...
   - Mientras el preparador procesa el firmware, se muestra la barra de progreso y la etiqueta de estado indica "Preparing firmware from <nombre_archivo>...".
...
| **Network MTU:** | Spin box | 1450 | 576–9000 bytes | Establece el tamaño máximo de paquete UDP VITA-49 saliente en bytes. El valor predeterminado 1450 es seguro para la mayoría de túneles VPN/SD-WAN. Almacenado en AppSettings como `NetworkMtu`. |
...
Use esta página para configurar los ajustes de transmisión, incluidos tiempos, enclavamientos, potencia máxima, modo de sintonía, visualización en el waterfall y comportamiento de seguimiento entre slice y TX.
...
|---|---|---|---|
| **Max Power:** | Spin box | — | Establece el límite máximo de potencia de TX a nivel de radio (0–100%). |
| **Tune Mode:** | Combo box | — | Selecciona cómo se comporta el botón de sintonía. |
| **Timings** | Spin box / Campo de texto | — | Tiempos de espera/retención de TX. |
...
| **TX Follows Active Slice** | Botón | Falso | TX sigue al slice activo. Mutuamente excluyente con 'Active Slice Follows TX'. Se deshabilita automáticamente durante una operación de Split. |
| **Active Slice Follows TX** | Botón | Falso | Cambia el slice activo cuando TX se mueve externamente (por ejemplo, WSJT-X o CAT). Mutuamente excluyente con 'TX Follows Active Slice'. |

### Campos de temporización de TX
...
La pestaña Antennas le permite asignar nombres amigables a cada puerto de antena de la radio.

### Controles de nombre de antena

| Control | Tipo | Notas |
|---|---|---|
| **ANT1 / ANT2 / XVTA / XVTB** | Campo de texto | Etiqueta personalizada para cada puerto de antena. Los nombres aparecen en el selector de antena del band scope y en el menú desplegable de antena de TX. |
...
5. Cuando la calibración se completa, el botón se habilita nuevamente y el campo de estado muestra el resultado.
...
| **Start** | Botón | Comienza la calibración. Restablece `freq_error_ppb` a 0, luego emite `radio pll_start`. Deshabilitado mientras está ocupado. |
| **Freq Offset (ppb):** | Spin box | Compensación de frecuencia manual en partes por billón. |
...
| **Audio Buffer:** | Campo de texto | Valor predeterminado: 200. Rango válido: 50–1000 ms. Aumenta el búfer de audio para jitter en VPN/SmartLink. |
...
| **Save to:** | Campo de texto | — | Carpeta para grabaciones guardadas (solo lado del cliente). Valor predeterminado: Documentos/AetherSDR/Recordings. |
...
| **Idle timeout:** | Spin box | 120 | Rango válido: 10–3600 seg. Segundos de silencio antes de que se detenga
