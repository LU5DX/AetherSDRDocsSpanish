...
| Control | Qué hace | Valor predeterminado | Valores/rango válido |
|---|---|---|---|
| **Port** | Selecciona o ingresa la ruta del dispositivo serie. | — | Puertos serie del sistema |
| **Refresh** | Vuelve a escanear en busca de puertos serie disponibles. | — | — |
...
| **Auto-open serial port on startup** | Vuelve a abrir el puerto configurado cuando se inicia AetherSDR. | Sin marcar | Marcado / Sin marcar |
...
El preparador de firmware detecta el formato del archivo automáticamente a partir de los primeros 8 bytes (magia OLE/MSI para `.msi`, cabecera PE/COFF MZ para `.exe`) y extrae la carga útil `.ssdr` sin necesidad de herramientas externas. Una barra de progreso y una etiqueta de estado se actualizan mientras se ejecuta la extracción.

Además, el botón **Check for Update** ya no se transforma en un botón **Download** cuando se encuentra una versión de firmware más reciente. En su lugar, cuando hay una actualización disponible, la etiqueta de estado le indica que descargue el instalador SmartSDR desde flexradio.com y luego haga clic en **Select Installer...** para prepararlo.
...
| **Upload Firmware** | Inicia la carga del firmware a la radio con una barra de progreso. | Habilitado solo después de que el preparador complete la extracción con éxito. |
...
Antes de comenzar la calibración, AetherSDR ahora restablece el error de frecuencia almacenado a cero (`radio set freq_error_ppb=0`) antes de enviar `radio pll_start`. Si el campo **Cal Frequency (MHz)** está vacío cuando hace clic en **Start**, la etiqueta de estado muestra *Enter cal frequency* en ámbar y la calibración no comienza.
...
El cuadro combinado **10 MHz Reference Source:** en la pestaña **RX** se ha actualizado para completarse y mostrarse con mayor precisión según el hardware detectado y el estado del oscilador en vivo.
...
El cuadro combinado se reconstruye dinámicamente. **Auto** siempre está presente. Aparecen entradas adicionales cuando cualquiera de las siguientes condiciones es verdadera para esa fuente:
...
La etiqueta de estado de bloqueo junto al cuadro combinado ahora muestra una descripción más detallada:
...
En todos los casos, el estado de bloqueo se agrega: *Locked* (mostrado en verde) o *Unlocked* (mostrado en rojo). Si la fuente activa es **External 10 MHz** pero no se detecta una referencia externa, *(not detected)* se agrega después del estado de bloqueo.

El color de la etiqueta es verde (`#00c040`) cuando está bloqueado, rojo (`#c04040`) cuando está desbloqueado y gris-azul (`#8aa8c0`) mientras se espera el estado.
...
La pestaña **Themes** se agregó en v0.9.3 en Radio Setup. Alberga la sección **Slice Colors**, que le permite reemplazar los colores de slice integrados de AetherSDR con una paleta completamente personalizada por slice.
...
| **Slice A–H color buttons** | Abre un selector de color para el slice correspondiente. | — | El color se aplica inmediatamente al hacer clic en Aceptar en el selector. |
| **Reset All to Defaults** | Restablece todos los colores de slice a los valores integrados de AetherSDR. | — | Solo está habilitado cuando **Custom colors** está seleccionado. |
