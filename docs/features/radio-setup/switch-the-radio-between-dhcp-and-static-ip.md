...
3. Opcionalmente, establezca un **Nickname**, **Callsign** o **Station Name** en los campos de texto. El **Station Name** identifica este cliente de AetherSDR ante otras estaciones multiFLEX; por defecto usa el nombre de host del SO si está vacío. Se almacena en AppSettings como `StationName`.
...
5. Para reiniciar el radio:
   ...
   - El botón está deshabilitado cuando el radio está desconectado; se vuelve a habilitar automáticamente cuando el radio se reconecta.
6. Para actualizar el firmware:
   ...
   - Haga clic en **Select Installer...** para abrir un selector de archivos. Seleccione el instalador o un archivo `.ssdr` previamente extraído.
...
- La etiqueta del botón **Select Installer...** cambió de **Browse .ssdr...** en v26.5.3.
...
4. Establezca **Network MTU:** como un valor de cuadro de giro (576-9000 bytes, predeterminado 1450). Esto establece el tamaño máximo de paquete UDP VITA-49 saliente. El valor predeterminado de 1450 es seguro para la mayoría de los túneles VPN/SD-WAN. Se almacena en AppSettings como `NetworkMtu`.
...
8. Vuelva a conectarse al radio en su nueva dirección usando `Settings > Connect to Radio...`.
...
2. Ajuste los cuadros de giro de **Timings** para los retrasos de ACC TX, TX Delay, RCA TX1, Timeout y TX2 en milisegundos.
   - **Timeout (sec):** Se muestra en segundos enteros; el radio almacena esto en milisegundos internamente.
...
7. Configure el seguimiento de slices:
   - **TX Follows Active Slice:** Botón pulsador (predeterminado False). Se almacena como `TxFollowsActiveSlice`. Mutuamente excluyente con **Active Slice Follows TX**. Se deshabilita automáticamente durante la operación de Split.
   - **Active Slice Follows TX:** Botón pulsador (predeterminado False). Se almacena como `ActiveFollowsTxSlice`. Cambia la slice activa cuando TX se mueve externamente (por ejemplo, WSJT-X o CAT).
...
3. Configure los ajustes de CW:
   - **Iambic:** Alterne para habilitar o deshabilitar el manipulador iambic en el radio. Mutuamente excluyente con el manipulador iambic del lado del radio; también controla el manipulador iambic local de software para tono lateral inferior a 5 ms.
   - **Iambic Mode: A / B:** Seleccione el modo Curtis iambic A o B. Se aplica tanto al radio como al manipulador de software local.
   - **Swap:** Alterne para intercambiar dit/dah.
   - **Sideband:** Seleccione LSB o USB para el tono de CW.
   - **CWX:** Alterne para habilitar la manipulación de macros CWX.
   - **Decode:** Alterne (predeterminado True) para habilitar la superposición de decodificación CW en el panadapter. Se almacena como `CwDecodeOverlay`.
...
- La pestaña etiquetada como "Antennas" aparece entre las pestañas RX y Filters.
...
9. Configure la grabación:
   ...
   - **Save to:** Campo de texto para la carpeta (solo lado del cliente). Por defecto en Documentos/AetherSDR/Recordings. Se almacena como `QsoRecordingDir`.
   ...
   - Establezca **Idle timeout:** (10-3600 seg, predeterminado 120) para los segundos de silencio antes de que se detenga la grabación. Se almacena como `QsoRecordingIdleTimeout`.
...
2. Vea la **Cables list / Status** detectada con estado Plugged/Unplugged por tipo de cable.
...
### Reconexión automática de periféricos y eliminación de IP manual
...
Si limpia el campo IP y hace clic en **Connect** mientras está desconectado, la IP y el puerto manuales guardados se eliminan de la configuración, lo que evita la reconexión automática al inicio. Si limpia el campo IP y cierra el diálogo de **Radio Setup** sin hacer clic en Connect/Disconnect, la configuración guardada también se limpia y cualquier conexión activa se desconecta.

### Notas específicas de TGXL

- Requerido para recuperar TUNE en firmware 4.2+.
- Cuando está conectado, el botón TUNE envía el comando `autotune` nativo directamente al TGXL en lugar de la ruta del lado del radio rota en el firmware 4.2.
...
Configure los puertos de muestreo de Predistorsión Adaptativa por antena TX. La pestaña está oculta a menos que el radio informe `apd configurable=1` (FLEX-8x00 con SmartSDR 4.2.18+).
