...
Los botones de antena en el applet Antenna Genius están codificados por color para mostrar el permiso de TX y RX en la banda actual. Esto permite ver de un vistazo qué antenas están disponibles para transmitir antes de seleccionar una.
...
- El applet debe estar conectado a un dispositivo. La etiqueta de estado debe indicar "Connected — \<name\> v\<version\>".
...
2. Lea el color del botón:
   ...
   - **Atenuado (no seleccionado, visualmente opaco)** — la antena no tiene permiso en la banda actual para TX ni RX.
3. Si necesita una antena con capacidad de TX, haga clic en un botón que no esté atenuado. Después de seleccionarlo, confirme que se ilumina en azul y no en ámbar.
...
Los botones también se deshabilitan y aparecen atenuados cuando la misma antena ya está seleccionada en el otro puerto. Consulte [Intercambiar radios que comparten el AG (las antenas en uso por el otro puerto están bloqueadas)](swap-radios-that-share-the-ag-antennas-in-use-by-the-other-port-are-locked-out.md) para obtener detalles sobre ese caso.
...
- Si utiliza el modo AUTO, el applet selecciona antenas según las reglas de seguimiento de banda. La misma codificación de colores se aplica a la antena seleccionada automáticamente. Consulte [Habilitar el modo AUTO para que el AG siga los cambios de banda del radio](enable-auto-mode-so-the-ag-follows-radio-band-changes.md).

## Solución de problemas

- **Todos los botones están atenuados después de conectar** — Es posible que el applet aún no haya recibido información de banda del radio. Confirme que el radio está sintonizado en una banda válida y que la etiqueta de estado muestra "Connected". Si el applet se abrió antes de que el radio estuviera en una banda, cambie de banda una vez para activar una actualización.
- **Los colores no se actualizan al cambiar de banda** — La actualización de seguimiento de banda requiere una conexión activa al dispositivo Antenna Genius. Verifique que la etiqueta de estado siga indicando "Connected — \<name\> v\<version\>" y que no se muestre ningún error.
