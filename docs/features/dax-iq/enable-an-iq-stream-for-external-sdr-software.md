...
El applet DAX IQ le permite activar uno o más de cuatro flujos IQ en su FLEX-8600 para que el software SDR externo pueda recibir datos IQ en bruto desde la radio. Cada flujo puede habilitarse de forma independiente con su propia frecuencia de muestreo.
...
| Botón Off/On (IQ 1–4) | `Off` | `Off`, `On` | `DaxIqEnabled1` – `DaxIqEnabled4` |
...
- El cuadro combinado de tasa se sincroniza con la tasa reportada por la radio una vez que el flujo está activo. Si la radio anula su selección, el cuadro reflejará el valor de la radio.
...
- **El botón cambia a On pero no llegan datos al software externo** — La solicitud de flujo puede haberse enviado antes de que la radio estuviera lista. Deshabilite el flujo haciendo clic en **On** para volver a **Off**, espere un momento, luego haga clic en **Off** nuevamente para re-habilitarlo.
- **El cuadro combinado de tasa se revierte después de habilitar** — La radio ha reportado una frecuencia de muestreo diferente para el flujo activo. El cuadro se sincroniza con el valor reportado por la radio; seleccione su tasa preferida antes de habilitar el flujo.
