# Habilite el servidor TCI para clientes Log4OM / SunSDR

El applet TCI ejecuta un servidor WebSocket que expone el control de la radio y el audio a software de terceros como Log4OM y herramientas SunSDR. Actívelo para permitir que esos clientes se conecten a AetherSDR a través del protocolo TCI.
...
1. Haga clic en el botón **TCI** de la barra lateral derecha. Se abre el panel del applet del servidor TCI.
...
3. Haga clic en **Enable**. El botón se vuelve verde cuando el servidor está en funcionamiento.
4. Verifique el indicador de estado a la izquierda de **Enable**. Muestra `:<puerto> (0 clients)` cuando el servidor está activo y en espera, y actualiza el número de clientes a medida que el software se conecta.

## Qué hace cada control

| Control                           | Predeterminado | Rango / Estados                                      |
|-----------------------------------|----------------|------------------------------------------------------|
| Campo de texto **Port**           | `50001`        | 1024–65535; valores no válidos se restablecen a `50001` |
...
| Etiquetas de asignación de slices RX/TX        | `—`            | `—` o `Slice <letra>`                                |
| Indicador de estado del servidor               | `(stopped)`    | `(stopped)`, `:<puerto> (N clients)`, `(port in use)` |

Las filas RX1–RX4 muestran qué slice impulsa cada canal TCI. La etiqueta muestra `Slice A`, `Slice B`, etc., según la asignación del canal DAX de cada slice. La fila TX muestra el slice TX activo actualmente.

## Consejos

- Para iniciar el servidor TCI automáticamente cada vez que se lance AetherSDR, vaya a `Settings > Autostart TCI with AetherSDR` y active ese elemento. Consulte [Autostart TCI on launch](autostart-tci-on-launch.md).
