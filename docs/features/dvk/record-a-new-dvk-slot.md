...
Use el panel Digital Voice Keyer para grabar su voz en uno de hasta 12 slots, que luego podrá reproducir al aire con un solo clic o pulsando una tecla F.
...
2. Haga clic en **● REC**. El botón se vuelve rojo y el indicador de estado cambia a **Recording**. La barra de progreso del slot aparece y avanza a medida que se captura el audio.
3. Hable su mensaje al micrófono.
4. Haga clic en **■ STOP** cuando haya terminado. El indicador de estado regresa a **Idle**, la barra de progreso desaparece y la etiqueta de duración del slot se actualiza de **Empty** para mostrar la duración grabada.
...
| Barras de progreso de slot | Muestra el progreso de grabación o reproducción en tiempo real. Ocultas cuando están inactivas. | — |
| **● REC** | Inicia la grabación en el slot seleccionado. Se vuelve rojo mientras está activo. | — |
| **■ STOP** | Detiene la grabación o la reproducción. | — |
| **▶ PLAY** | Reproduce el slot seleccionado al aire. | — |
...
- Puede renombrar un slot después de grabar haciendo clic derecho en su fila y eligiendo renombrar en el menú contextual. La etiqueta del nombre del slot es editable directamente.
...
- **● REC no hace nada al hacer clic** — Ningún slot está seleccionado. Haga clic primero en una fila de slot y luego en **● REC**.
...
- [Reproducir un slot de voz almacenado](play-a-stored-voice-keyer-slot.md)
