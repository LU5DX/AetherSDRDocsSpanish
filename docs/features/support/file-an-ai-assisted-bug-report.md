...
Utilice el flujo de informe de error asistido por IA para obtener ayuda al redactar un issue de GitHub claro y completo. AetherSDR copia un mensaje de diagnóstico previamente rellenado —que incluye su versión, sistema operativo y radio conectada— en el portapapeles, y luego lo guía a través de un asistente de IA y el formulario de issue de GitHub.

## Antes de comenzar

- Reproduzca el problema al menos una vez para poder describir lo que ocurrió.
- Si desea adjuntar registros de diagnóstico, limpie el registro y reproduzca el problema primero para que el registro contenga solo la salida relevante. Consulte [Limpiar el registro antes de reproducir un error](clear-the-log-before-reproducing-a-bug.md).
...
2. Haga clic en `File an Issue`.
   AetherSDR crea un paquete de soporte (registros y configuración) y copia un mensaje de diagnóstico en el portapapeles. El mensaje incluye su versión de AetherSDR, versión de Qt, sistema operativo e información de la radio si está conectada.
3. En el cuadro de diálogo de Informe de Error Asistido por IA que aparece, haga clic en el servicio de IA que desea usar: `Claude`, `ChatGPT`, `Gemini`, `Grok` o `Perplexity`.
...
- El paquete de soporte se crea cuando hace clic en `File an Issue`, antes de interactuar con cualquier IA. Si reproduce el problema después de abrir el cuadro de diálogo, haga clic en `Close`, limpie el registro, reproduzca el error y luego inicie el flujo nuevamente para que el paquete contenga registros recientes.
- Si cierra el cuadro de diálogo de Informe de Error Asistido por IA y necesita presentar el issue más tarde, haga clic en `Submit Bug Report` desde una nueva sesión de `File an Issue` para volver a abrir el formulario de GitHub y la carpeta del paquete.

## Solución de problemas

- **Aparece la advertencia "No se pudo crear el paquete de soporte"** — AetherSDR no pudo escribir el paquete en el disco. Verifique que tenga permisos de escritura en su directorio de inicio y que haya espacio disponible en disco, luego intente de nuevo.
...
- **La información de la radio muestra "no conectada" en el mensaje** — La radio no estaba conectada cuando hizo clic en `File an Issue`. Agregue el modelo de radio y la versión de firmware manualmente en el chat de IA después de pegar el mensaje.
...
El cuadro de diálogo de Soporte y Diagnóstico (`Help > Support...`) proporciona visualización de registros, control de categorías de registro y acceso a herramientas de soporte. El cuadro de diálogo recuerda su tamaño y posición entre sesiones.
...
| Etiqueta de ruta de registro | Muestra la ruta del archivo de registro actual. |
...
| Control | Qué hace |
|---|---|
| File an Issue | Inicia el flujo de Informe de Error Asistido por IA. |
