# Resumen del Aetherial De-Esser

El Aetherial De-Esser es un procesador del lado del cliente disponible tanto en instancias de TX como de RX. Reduce la sibilancia áspera (sonidos "S" y "T") monitorizando una banda de frecuencia estrecha y atenuándola cuando el nivel de señal en esa banda supera un umbral establecido.
...
- Para abrir la instancia de RX, use el icono del **Aetherial Audio Channel Strip** para el canal de recepción, luego haga clic en la etapa De-Ess. El panel se abre con el título "Aetherial De-Esser — RX".
...
|---|---|---|
| TX (applet acoplado) | Haga clic en el icono De-Esser en el Panel de Applets | "Aetherial De-Esser" |
...
Cada instancia mantiene ajustes independientes para Freq, Q, Thresh, Amount, Attack, Release y Slope. El applet acoplado omite los mandos de Attack y Release.

## Qué hace cada control

| Control | Valor predeterminado | Rango válido | Comportamiento |
|---|---|---|---|
| **Freq** | 6000 Hz | 1000 – 12000 Hz | Mapeo logarítmico (1000 * 12^n). Establece la frecuencia central de la banda de sibilancia. La etiqueta muestra '6.0 kHz' por encima de 1 kHz, 'N Hz' por debajo. |
...
| **Attack** | 1.0 ms | 0.1 a 30.0 ms | Mapeo exponencial (0.1 * 300^n). Establece qué tan rápido responde el de-esser una vez que la sibilancia cruza el umbral. Presente en el StripDeEssPanel del Channel Strip (RX y TX). El ClientDeEssApplet acoplado omite este mando. |
| **Release** | 100 ms | 10.0 a 500.0 ms | Mapeo exponencial (10 * 50^n). Establece qué tan rápido regresa la ganancia después de que la sibilancia cae por debajo del umbral. Presente en el StripDeEssPanel del Channel Strip (RX y TX). El ClientDeEssApplet acoplado omite este mando. |
| **Slope** | 24 dB/oct (2 etapas) | 12 / 24 / 36 / 48 dB/oct (1 a 4 etapas) | Cicla el conteo de cascada del paso banda de la cadena lateral. Cada etapa añade 12 dB/oct de caída fuera de la banda sibilante. Mayor slope = muesca efectiva más estrecha, menos daño colateral en la banda media en frases con muchas "S". Presente en el StripDeEssPanel flotante (columna izquierda, abajo). La etiqueta muestra 'N dB/oct'. Presente tanto para rutas TX como RX. Se persiste como `ClientDeEssTxSlopeStages` / `ClientDeEssRxSlopeStages`. |

### Edición de valor en línea

Cada mando admite la edición de valor en línea. Haga clic en el texto del valor mostrado para abrir un pequeño campo de entrada de texto. Escriba un nuevo valor y presione **Enter** o haga clic fuera del campo para confirmar. El valor se ajusta automáticamente al rango válido del mando. Presione **Escape** para cancelar la edición y revertir al valor anterior. Esta función funciona de la misma manera en el applet acoplado y en los paneles del channel strip.

## Botón Slope

El botón **Slope** en el panel del Channel Strip le permite ajustar la pendiente del filtro de la cadena lateral:

- Haga clic en el botón para ciclar a través de 12 → 24 → 36 → 48 dB/oct (de 1 a 4 biquads paso banda en cascada).
- Cada clic aumenta la caída en 12 dB/oct fuera de la banda sibilante.
- Los valores de pendiente más altos crean una muesca más estrecha alrededor de la frecuencia central, reduciendo la atenuación colateral del habla en la banda media.
- La etiqueta del botón se actualiza para mostrar la configuración actual (por ejemplo, "24 dB/oct").
- La curva de respuesta de la cadena lateral se actualiza para reflejar la nueva pendiente.
- La configuración de Slope se guarda de forma independiente para las rutas TX y RX.
...
Los ajustes de RX utilizan un conjunto de claves paralelo (`ClientDeEssRx*`). Los ajustes se guardan al ajustar cualquier mando o cerrar el panel.

## Tematización

El panel del de-esser y sus mandos utilizan el sistema de temas de la aplicación. Los colores de los mandos (anillo de fondo, arco de primer plano, manija del puntero, texto de etiqueta, texto de valor) se obtienen de las claves de color del tema:

- `color.knob.background` — anillo de fondo del mando
- `color.knob.foreground` — arco de valor del mando
- `color.knob.handle` — línea del puntero del mando
- `color.text.secondary` — texto de la etiqueta del mando
- `color.text.primary` — texto del valor del mando
...
- `color.accent.warning` — resplandor de la esfera de frecuencia central
...
- Comience con **Freq** en el valor predeterminado de 6.0 kHz y barra lentamente mientras pronuncia palabras sibilantes. Observe la barra de reducción de ganancia — la desviación máxima indica que ha encontrado la frecuencia de sibilancia máxima.
...
- Establezca **Thresh** para que la barra de reducción de ganancia se mueva solo en sonidos genuinos "S" y "T", no en vocales o consonantes normales.
- La marca de −6 dB en la barra de reducción de ganancia señala el valor predeterminado de **Amount**. Mantener la reducción cerca de esa marca generalmente produce resultados transparentes. Cantidades mayores están disponibles, pero pueden hacer que el efecto sea audible como bombeo o ceceo.
- Use el botón **Slope** para ajustar la pendiente del filtro. Comience con 24 dB/oct y auméntelo si escucha demasiada atenuación en la banda media en frases con mucha sibilancia.
- Use configuraciones diferentes para TX y RX — es posible que necesite un de-essing más agresivo en recepción que en transmisión, o viceversa.
- Cuando la etapa está en bypass, el panel se atenúa notablemente. Si el panel aparece atenuado y no escucha el de-essing, verifique que la etapa DESS no esté en bypass en el widget CHAIN.
