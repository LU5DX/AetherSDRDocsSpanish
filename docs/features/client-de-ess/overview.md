...
- Para abrir la instancia de RX, use el icono **Aetherial Audio Channel Strip** del canal de recepción y luego haga clic en la etapa De-Ess. El panel se abre con el título "Aetherial De-Esser — RX".
...
Cuando la etapa de-esser está puenteada, todo el panel se renderiza con opacidad reducida (aproximadamente 55 %) para dar una indicación visual clara de que la etapa está inactiva. Para puentear o reactivar el de-esser, use el gesto de un solo clic en la etapa DESS dentro del widget CHAIN.
...
| TX (canal) | Abra el canal TX, haga clic en la etapa De-Ess | "Aetherial De-Esser — TX" |
| RX (canal) | Abra el canal RX, haga clic en la etapa De-Ess | "Aetherial De-Esser — RX" |
...
| **Attack** | 1.0 ms | 0.1 a 30.0 ms | Mapeo exponencial. Qué tan rápido responde el de-esser una vez que la sibilancia cruza el umbral. Solo en el panel Channel Strip (RX y TX). |
| **Release** | 100 ms | 10.0 a 500.0 ms | Mapeo exponencial. Qué tan rápido regresa la ganancia después de que la sibilancia cae por debajo del umbral. Solo en el panel Channel Strip (RX y TX). |

### Edición de valor en línea

Cada control admite edición de valor en línea. Haga clic en el texto del valor mostrado para abrir un pequeño campo de entrada de texto. Escriba un nuevo valor y presione **Enter** o haga clic fuera del campo para confirmarlo. El valor se ajusta automáticamente al rango válido del control. Presione **Escape** para cancelar la edición y revertir al valor anterior. Esta función funciona de la misma manera en el applet acoplado y en los paneles del channel strip.
...
|---|---|
| `ClientDeEssTxFrequencyHz` | Frecuencia central TX |
| `ClientDeEssTxQ` | Factor de ancho de banda TX |
| `ClientDeEssTxThresholdDb` | Umbral de cadena lateral TX |
| `ClientDeEssTxAmountDb` | Atenuación máxima TX |
| `ClientDeEssTxAttackMs` | Tiempo de ataque TX |
| `ClientDeEssTxReleaseMs` | Tiempo de liberación TX |
| `ClientDeEssTxEnabled` | Estado habilitado TX |
...
- Comience con **Freq** en el valor predeterminado de 6.0 kHz y barra lentamente mientras pronuncia palabras sibilantes. Observe la barra de reducción de ganancia — la deflexión máxima indica que ha encontrado la frecuencia de sibilancia máxima.
...
- Establezca **Thresh** para que la barra de reducción de ganancia se mueva solo con sonidos genuinos de "S" y "T", no con vocales o consonantes normales.
...
- Cuando la etapa está puenteada, el panel se atenúa notablemente. Si el panel aparece atenuado y no escucha el de-essing, verifique que la etapa DESS no esté puenteada en el widget CHAIN.
