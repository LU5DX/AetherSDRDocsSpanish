...
En esta página se explica cómo ajustar el mando Thresh para que el de-esser actúe únicamente sobre la sibilancia genuina y deje intacto el habla más suave. Un umbral bien colocado marca la diferencia entre un de-essing transparente y un bombeo audible.
...
- Necesita una forma de transmitir o monitorizar su propio audio de TX para que el habla genuina llegue a la cadena lateral del de-esser.
...
3. Observe la barra de Gain-reduction. Si muestra un relleno rojo suave durante vocales y consonantes normales (no solo en picos de 'S'), el umbral está demasiado bajo. Si nunca se mueve durante los sonidos de 'S' fuertes, el umbral está demasiado alto.
...
5. Luego, gire Thresh lentamente en sentido contrario a las agujas del reloj (hacia −60.0 dB) hasta que la barra de Gain-reduction comience justo a llenarse en sus picos de 'S' más fuertes, y no más.
6. Verifique: hable normalmente durante una frase completa. La barra de Gain-reduction debe estar vacía la mayor parte del tiempo y llenarse brevemente solo en sibilantes duras.
...
**Thresh** — el nivel por encima del cual el de-esser comienza a atenuar la banda de sibilancia. Subir este valor (hacia 0.0 dB) hace que el de-esser actúe solo sobre las sibilancias más fuertes. Bajarlo (hacia −60.0 dB) hace que el de-esser se active con señales progresivamente más silenciosas.

**Gain-reduction bar** — una barra horizontal de color rojo suave que se llena desde la derecha para mostrar la atenuación actual. La escala va de 0 a 24 dB. Una marca indica la posición de −6 dB, que es el valor predeterminado de Amount. La barra se actualiza aproximadamente 30 veces por segundo.
...
**Attack** — la rapidez con la que responde el de-esser una vez que la sibilancia supera el umbral (0.1 a 30.0 ms, mapeo exponencial). Presente en el StripDeEssPanel del Channel Strip tanto para RX como para TX. El ClientDeEssApplet acoplado omite este mando.

**Release** — la rapidez con la que la ganancia regresa después de que la sibilancia cae por debajo del umbral (10.0 a 500.0 ms, mapeo exponencial). Presente en el StripDeEssPanel del Channel Strip tanto para RX como para TX. El ClientDeEssApplet acoplado omite este mando.

**Slope** — establece el número de cascadas del filtro paso banda de la cadena lateral. Cada etapa añade 12 dB/oct de caída fuera de la banda sibilante. Una pendiente más alta = muesca efectiva más estrecha, menos daño colateral en la banda media en frases con muchas 'S'. Haga clic en el botón para recorrer 12 → 24 → 36 → 48 dB/oct (1 a 4 etapas). Presente en el StripDeEssPanel del Channel Strip (columna izquierda, parte inferior). Se conserva como `ClientDeEssTxSlopeStages` para TX y `ClientDeEssRxSlopeStages` para RX.
...
El punto de frecuencia central descansa sobre el pico de la curva, marcando la frecuencia central de sibilancia actualmente sintonizada.
...
Cada mando de sintonización (Freq, Q, Thresh, Amount, Attack, Release) admite la entrada directa de valores. Haga clic en el valor de texto mostrado para activar una superposición de editor en línea. El editor aparece como un campo de texto con borde cian sobre un fondo oscuro.

- **Enter** — confirma el valor escrito y cierra el editor.
- **Hacer clic en otro lugar (pérdida de foco)** — confirma el valor escrito y cierra el editor.
- **Escape** — descarta el valor escrito y vuelve al valor anterior.

El editor en línea acepta formatos de número según la configuración regional (por ejemplo, "12,5" en configuraciones regionales con coma decimal). También tolera texto adicional como sufijos de unidad ("6 kHz", "−30 dB") eliminando los caracteres no numéricos antes del análisis. Si el análisis falla, el editor vuelve silenciosamente al último valor mostrado válido.
...
- Utilice valores de Slope más altos (36 o 48 dB/oct) cuando tenga pasajes densos en sibilantes para minimizar la atenuación colateral de la banda media del habla. Una pendiente de 24 dB/oct es un buen punto de partida para la mayoría de las voces.
