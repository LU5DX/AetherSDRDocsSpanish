...
El control deslizante Zoom en el applet de forma de onda escala el eje de amplitud de la visualización. Aumentar el zoom estira verticalmente las señales pequeñas para que sean más fáciles de leer; disminuirlo evita que los artefactos de recorte oculten la traza en señales fuertes. El control deslizante Window controla la ventana de tiempo que se muestra en la visualización de forma de onda.

## Antes de comenzar

- El applet de forma de onda debe estar visible. Si no lo está, haga clic en el botón de bandeja WAVE en la barra lateral derecha para mostrarlo.
...
El control deslizante Window selecciona entre pasos discretos de ventana de tiempo. Los primeros dos puntos proporcionan detalles de subsegundo (240 ms y 480 ms), seguidos de incrementos de un segundo desde 1 segundo hasta 10 segundos. Cada punto es una parada deliberada, no un rango continuo.
...
- La configuración de zoom y ventana se aplica por igual a las rutas de RX y TX. El tinte de dirección (frío para RX, cálido para TX) aún distingue qué ruta está activa independientemente del nivel de zoom.
...
- El intervalo de discriminación de clics utilizado para distinguir un solo clic de un doble clic respeta el valor que estableció en Radio Setup → Interaction Settings. Los cambios en esa configuración surten efecto de inmediato sin necesidad de reiniciar AetherSDR.
...
- **El control deslizante Zoom vuelve a su posición después de arrastrarlo** — Esto puede suceder si no llega audio y la visualización muestra el marcador de posición sin audio. El valor del control deslizante aún se guarda; surte efecto tan pronto como se reanude el audio.
- **El zoom se restablece después de reiniciar AetherSDR** — Verifique que el valor se esté conservando. Si la aplicación se cerró de manera anormal, es posible que la configuración `WaveApplet_ZoomPercent` no se haya escrito. Establezca el control deslizante al valor deseado después de un inicio limpio.
- **La configuración de Window cambió inesperadamente después de una actualización** — Si actualiza desde una versión anterior que usaba la configuración `WaveApplet_TimeWindowSec` (1–20 s lineal), el valor se migra automáticamente al paso discreto más cercano en `WaveApplet_TimeWindowMs`. Verifique la configuración y ajústela si es necesario.
- **El mensaje del marcador de posición sin audio cambió** — Cuando no llega audio de RX, la visualización ahora muestra "Enable PC Audio" en lugar de "no RX audio". Esto indica que debe habilitar el audio de PC en la configuración de la radio o la configuración de audio. Para TX, el mensaje aún muestra "no TX audio".
