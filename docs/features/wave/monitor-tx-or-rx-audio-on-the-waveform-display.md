# Monitorear audio de TX o RX en la visualización de forma de onda
...
- AetherSDR debe estar en ejecución. No se requiere conexión de radio: el applet muestra el audio del motor de audio local.
...
5. Si no ha llegado audio durante un segundo, la pantalla muestra un marcador de posición "no RX audio" o "no TX audio" en lugar de una traza vacía.
...
7. En el cajón de configuración, use el cuadro combinado View para elegir una visualización: **Scope**, **Envelope**, **History** o **Bands**. La opción predeterminada es **Scope**.
...
9. Use el control deslizante FPS para establecer la frecuencia con la que se actualiza la pantalla (rango 5–30 Hz, valor predeterminado 24). Los valores más bajos reducen la carga de la CPU.
...
- Cuando ocurre un recorte, las columnas afectadas se resaltan y aparece un contador CLIP N en el encabezado. Reduzca el nivel de excitación de audio o disminuya el valor de Zoom para que la señal vuelva al rango.
...
- El control deslizante Window utiliza pasos discretos en lugar de ajuste continuo. Cada muesca proporciona un valor de ventana de tiempo específico. Los primeros tres pasos (240 ms, 480 ms, 1 s) ofrecen detalle de subsegundo; los pasos restantes aumentan en incrementos de 1 segundo hasta 10 s.
...
- **El botón de bandeja WAVE no aparece** — El panel de applets puede estar oculto. Actívelo mediante `View > Applet Panel`. Si el panel está visible pero falta WAVE, use `View > Reset Applet Order` para restaurar la disposición predeterminada de los applets.
