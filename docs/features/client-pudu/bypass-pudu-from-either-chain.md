# Excitador Aetherial PUDU

El widget CHAIN le permite omitir la etapa PUDU en la cadena de TX, la cadena de RX, o ambas, sin abrir el editor de PUDU. Utilícelo cuando desee comparar el efecto A/B o eliminarlo temporalmente de la ruta de la señal.
...
3. Repita en el widget CHAIN de la otra cadena si desea omitir tanto TX como RX de forma independiente.

El estado de omisión se persiste: `ClientPuduTxEnabled` para la cadena TX y `ClientPuduRxEnabled` para la cadena RX.

## Consejos

- Un solo clic en el widget CHAIN omite la etapa. Un doble clic abre el editor de PUDU sin marco ("Aetherial Voice Processor — TX" o "Aetherial RX Poodoo — RX") sin cambiar el estado de omisión.
- Cuando la etapa está omitida, todo el mosaico PUDU se atenúa a una opacidad reducida (aproximadamente 55 %), coincidiendo con el efecto de atenuación utilizado en la visualización de la curva EQ. Esta señal visual es consistente, ya sea que omita desde el widget CHAIN o desde el editor.
- El logotipo de AetherVoice dentro del editor pulsa con el RMS de la señal procesada solo cuando la etapa está habilitada. Si el logotipo está estático, la etapa está omitida.
...
1. En el widget CHAIN para la cadena TX o RX, haga doble clic en el bloque de la etapa PUDU.
   - El editor de TX se abre con el título de ventana "Aetherial Poodoo — TX".
   - El editor de RX se abre con el título de ventana "Aetherial Poodoo — RX".
...
El editor etiqueta la primera perilla de Body como "Poo / Drive", la etiqueta del grupo Clarity es "Clarity" (no "Doo"), y la etiqueta de la perilla Air es "Air". Las claves de configuración subyacentes permanecen sin cambios.
...
|------------------------|----------------|-----------------|----------------------------|------------------------------------------------------------------|
| Even                   | —              | —               | `ClientPuduTxMode`         | Selecciona el modelado asimétrico de linaje Aphex (armónicos pares). Ámbar cuando está marcado. Exclusivo con Odd. |
| Odd                    | —              | —               | `ClientPuduTxMode`         | Selecciona el modelado simétrico tanh de linaje Behringer (armónicos impares). Exclusivo con Even. |
...
- **Even**: Color ámbar PooDoo cuando está marcado. Modelado asimétrico de linaje Aphex: predominantemente armónicos pares, más cálido, con saturación LF Big Bottom.
- **Odd**: Modelado simétrico tanh de linaje Behringer: armónicos impares puros, más brillante, con un compresor de graves feed-forward.

Los dos botones de opción son mutuamente excluyentes.

## Integración de temas

Los colores de las perillas (anillo de fondo, arco de primer plano, manija del puntero, texto de la etiqueta, texto del valor) ahora provienen del espacio de nombres de tema `color.knob.*`, con el contenedor de applet PUDU `applet/pudu` proporcionando una anulación de color de perilla ámbar en primer plano. El color del texto de la etiqueta del corchete sigue `color.text.primary` del tema activo.
...
2. Escriba el valor deseado. Puede incluir unidades (dB, Hz, %), pero se ignoran. Se aceptan separadores decimales sensibles a la configuración regional (coma o punto).
...
- [Ajustar Body Drive para la sonoridad de baja frecuencia](dial-poo-drive-for-lf-thickness.md)
