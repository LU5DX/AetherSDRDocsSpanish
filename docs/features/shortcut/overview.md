...
AetherSDR permite asignar atajos de teclado a las acciones de la aplicación y editar esas asignaciones en cualquier momento. Use el diálogo Keyboard Shortcuts para ver, reasignar, borrar o restablecer las asignaciones sin necesidad de reiniciar la aplicación.
...
Abra el editor en `View > Configure Shortcuts...`. El diálogo tiene dos áreas principales: un mapa visual del teclado en la parte superior y una tabla de acciones filtrable en la parte inferior.

**Mapa del teclado** — Un diseño QWERTY que muestra todas las teclas. Las teclas con acciones asignadas están codificadas por color según la categoría. Una leyenda debajo del mapa indica qué color corresponde a cada categoría. Haga clic en cualquier tecla para seleccionarla; el panel debajo del mapa se actualiza para mostrar qué está asignado a esa tecla.
...
- `Filter:` reduce la tabla al escribir cualquier parte del nombre de una acción o del nombre de una categoría.
...
- La tabla de acciones siempre muestra tanto la tecla actual como la predeterminada para cada acción, de modo que pueda ver de un vistazo qué ha cambiado.
- Seleccionar una fila en la tabla de acciones y seleccionar una tecla en el mapa del teclado son operaciones independientes. Para asignar un atajo, seleccione primero una tecla en el mapa y luego elija la acción desde el cuadro combinado `Action:`.
