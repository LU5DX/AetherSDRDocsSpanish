...
El control CAT permite que programas externos de registro y concursos controlen el FLEX-8600 a través de interfaces compatibles con rigctld. AetherSDR ejecuta hasta cuatro servidores independientes — uno por slice (A, B, C, D) — mediante TCP, puertos serie virtuales (PTY), o ambos simultáneamente.
...
Los programas externos se conectan al canal que controla el slice con el que desean operar. Varios clientes pueden conectarse al mismo canal simultáneamente; la fila de estado de cada canal muestra el número actual de clientes.
...
- Las rutas de los enlaces simbólicos PTY (`/tmp/AetherSDR-CAT-A` hasta `/tmp/AetherSDR-CAT-D`) son fijas hasta que el PTY se inicia; una vez en funcionamiento, la ruta real reportada por el sistema aparece en la fila del canal.
...
- [Habilitar CAT PTY para que aplicaciones en Linux/macOS puedan abrir un puerto CAT de tipo serie](enable-cat-pty-so-linux-macos-apps-can-open-a-serial-style-cat-port.md)
