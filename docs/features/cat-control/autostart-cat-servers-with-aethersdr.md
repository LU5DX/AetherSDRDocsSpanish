...
- AetherSDR debe estar conectado a una radio FLEX-8600 para que los servidores CAT puedan funcionar. La configuración de inicio automático se guarda independientemente, pero los servidores solo se activan una vez que se establece una conexión con la radio.
...
- Las rutas de enlaces simbólicos PTY se encuentran en directorios de tiempo de ejecución por usuario por seguridad:
  - Linux: `$XDG_RUNTIME_DIR/aethersdr/cat-A` hasta `cat-D`
  - macOS: `~/Library/Caches/AetherSDR/cat-A` hasta `cat-D`

## Pasos

### Activar inicio automático para servidores TCP
...
### Activar inicio automático para puertos serie virtuales PTY

1. Abra `Settings > Autostart CAT with AetherSDR`.
2. Haga clic en el elemento para marcar la casilla junto a él.

AetherSDR creará ahora los enlaces simbólicos PTY en las rutas de tiempo de ejecución por usuario automáticamente en el próximo inicio:
- Linux: `$XDG_RUNTIME_DIR/aethersdr/cat-A` hasta `cat-D`
- macOS: `~/Library/Caches/AetherSDR/cat-A` hasta `cat-D`
...
| Filas de canales A/B/C/D | Indicador    | `(stopped)`    |

El indicador de ruta PTY por canal muestra:
- `stopped` cuando no hay ningún PTY activo
- La ruta completa del enlace simbólico (por ejemplo, `/run/user/1000/aethersdr/cat-A` en Linux o `~/Library/Caches/AetherSDR/cat-A` en macOS) cuando el PTY está en ejecución

## Consejos

- La alternancia `Enable TCP` en el applet refleja la configuración `AutoStartRigctld`. La alternancia `Enable TTY` refleja la configuración `AutoStartCAT`. Alternar cualquiera de los botones en el applet actualiza la preferencia de inicio automático al mismo tiempo, por lo que puede usar los botones del applet en lugar del menú de Ajustes si lo prefiere.
- Si cambia el puerto `Base` después de activar el inicio automático, el nuevo puerto se guarda en `CatTcpPort` y los servidores en ejecución se reinician en la nueva base inmediatamente. El valor guardado también se utiliza en el próximo inicio automático.
- En v26.5.3, AetherSDR incluye una implementación nativa de control Hamlib NET rigctl, reemplazando la necesidad de un puente rigctld independiente. Las slices A–H exponen cada una un TTY (ruta de tiempo de ejecución por usuario) y un puerto TCP (4532–4539).
...
- **Los servidores no se inician después del inicio aunque el inicio automático esté activado** — La radio debe estar conectada antes de que los servidores se activen. Confirme el estado de la conexión en la barra de título y vuelva a intentarlo una vez conectado.
...
- **Puerto ya en uso** — Si otra aplicación ocupa un puerto en el rango Base–Base+3, el servidor correspondiente fallará en silencio. Cambie el valor de `Base` en el applet de Control CAT a un rango de puertos no utilizado y vuelva a habilitar TCP.
- **Los colores de las insignias de canal parecen incorrectos** — Los colores de las slices se gestionan dinámicamente. Si las insignias muestran colores inesperados, desconéctese y vuelva a conectarse a la radio para que se actualicen las asignaciones de color de las slices.
...
- [Activar CAT PTY para que las aplicaciones Linux/macOS puedan abrir un puerto CAT tipo serie](enable-cat-pty-so-linux-macos-apps-can-open-a-serial-style-cat-port.md)
