...
- **Los enlaces simbólicos PTY no aparecen** — `Enable TTY` está disponible solo en Linux y macOS. Confirme que hizo clic en `Enable TTY` y que AetherSDR tiene acceso de escritura al directorio de tiempo de ejecución del usuario. En Linux esto suele ser `$XDG_RUNTIME_DIR/aethersdr/`; en macOS `~/Library/Caches/AetherSDR/`.
