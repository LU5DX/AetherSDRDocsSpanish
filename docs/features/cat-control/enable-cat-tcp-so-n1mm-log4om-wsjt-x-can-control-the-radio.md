# Habilitar CAT TCP para que N1MM, Log4OM y WSJT-X puedan controlar la radio

El applet de Control CAT ejecuta hasta cuatro servidores TCP compatibles con rigctld, uno por canal de slice (A–D), para que los programas externos de registro y contesto puedan controlar la radio a través de un socket de red. Use esta página para iniciar esos servidores y apuntar su software de registro al puerto correcto.
...
1. Haga clic en el botón **CAT** de la barra lateral derecha para abrir el applet de Control CAT. El applet está oculto de forma predeterminada.
2. Verifique el valor en el campo **Base**. El valor predeterminado es `4532`. Cámbielo si es necesario (rango válido: 1024–65535). Presione Enter o haga clic fuera del campo para confirmar; los valores fuera de rango vuelven automáticamente a `4532`.
...
5. En su software de registro o contesto (N1MM, Log4OM, WSJT-X o similar), configure la conexión CAT como **rigctld (net)** e ingrese `localhost` (o la dirección IP del equipo con AetherSDR) y el puerto del canal que desea controlar; por ejemplo, `4532` para el canal A o `4533` para el canal B.
...
- **El contador de clientes no aumenta** — El software se conectó pero se desconectó de inmediato. Verifique que su software de registro esté configurado en modo **rigctld** (red), y no en modo serie directo o modo Hamlib que espera un protocolo diferente.
