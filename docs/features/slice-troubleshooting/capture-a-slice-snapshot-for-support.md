...
El diálogo Slice Troubleshooting captura una instantánea en un momento determinado de cada slice, panadapter, transverter y canal DAX del radio conectado. Úselo para recopilar información antes de enviar un informe de error o solicitar soporte técnico.
...
2. Revise los problemas detectados en la pestaña **Issue Summary**. Cada entrada es una descripción en lenguaje sencillo de un posible problema, como audio faltante, silencio bloqueado, antena faltante o una configuración de transverter no válida.
3. Revise los datos sin procesar en la pestaña **JSON** si necesita el detalle completo o desea adjuntarlos a un informe.
...
|---|---|---|
| **Issue Summary** | Pestaña | Muestra una lista de viñetas en lenguaje sencillo con los problemas detectados, incluyendo audio faltante, silencio bloqueado, antena faltante y problemas de validez del transverter. |
...
- Capture la instantánea antes y después de cambiar la configuración del slice si está tratando de aislar un problema. Use **Refresh Snapshot** entre capturas para actualizar los datos.
