...
- Tenga disponible un archivo de registro ADIF si desea usar la coloración por DXCC.
...
Cada fuente tiene un conmutador **Auto-connect on startup** o **Auto-start on startup**. Cuando está habilitado, esa fuente se conecta o inicia automáticamente cada vez que se lanza AetherSDR, sin intervención manual. Las claves guardadas son `ClusterAutoConnect`, `RbnAutoConnect`, `WsjtxAutoStart`, `SpotCollectorAutoStart`, `PotaAutoStart` y `FreeDvAutoStart`.

### Pestaña Spot List

La pestaña **Spot List** muestra una tabla unificada y ordenable de todos los spots activos de todas las fuentes activas. Las columnas son: Time, Freq (kHz), DX Call, Mode, Comment, Spotter, Band y Source. Las casillas de verificación por banda en **Bands:** activan o desactivan la visibilidad de cada banda de aficionados. Haga clic en **Clear** para vaciar la lista actual. Haga doble clic en cualquier fila para sintonizar el VFO activo en la frecuencia de ese spot y, cuando el spot contenga información de modo (por ejemplo, CW, FT8 o RTTY), cambie automáticamente el slice a ese modo.
...
| **Imported: (estadísticas DXCC)** | Muestra el conteo de QSO y entidades cuando se carga un registro. | (ningún registro cargado) |
...
**Spot Lines:** dibuja una línea vertical desde la línea base del espectro hasta cada etiqueta de spot. Desactívelo durante concursos para reducir el desorden visual. La configuración se guarda como `IsSpotsLinesEnabled` y su valor predeterminado es Habilitado.

**Auto:** el valor predeterminado es **Habilitado** (`SpotAutoSwitchMode` predeterminado en `True`). Si anteriormente dependía de que el modo automático estuviera desactivado por defecto, verifique esta configuración después de actualizar.
...
2. Complete un indicativo y un cuadrado de cuadrícula válidos en los campos **Callsign:** y **Grid Square:** (ver más abajo). La casilla de verificación se niega a habilitarse si alguno de los campos está en blanco o no se puede resolver.
3. Marque **Enable FreeDV Reporter reporting when RADE is active** (`FreeDvAutoReport`). Si el indicativo o la cuadrícula no se pueden resolver, aparece un diálogo de advertencia y la casilla de verificación vuelve al estado desmarcado.

> **Nota:** Los datos del Reporter se publican en un mapa público compartido por la comunidad. No habilite el reporte con valores temporales.
...
La versión V0.9.4 agrega una superposición de barrido de ROE que traza la ROE en función de la frecuencia directamente en el panadapter. Una fuente externa (por ejemplo, una integración de analizador de antenas) proporciona los datos llamando a `setSwrSweepPoints()`. El panadapter representa la curva mediante la capa interna `drawSwrSweep()`.
...
| `running` | `bool` | `false` | Pase `true` mientras un barrido está en curso para indicar una traza viva e incompleta. |
...
El diálogo de SpotHub y todas sus etiquetas de estado de conexión ahora usan colores compatibles con el tema. El contenedor del diálogo está registrado como `dialog/dxCluster` para el estilo del tema. Las etiquetas de estado de conexión usan:

| Estado | Token de color del tema |
|---|---|
| Conectado | `{{color.accent}}` |
| Desconectado | |
