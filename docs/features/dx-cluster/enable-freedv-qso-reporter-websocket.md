...
2. Revise el campo **Grid Square:**.
   - Si su radio tiene hardware GPS y **Use GPS** está marcado (valor predeterminado en modelos con GPS), el campo se rellena previamente desde la posición fija GPS de la radio y es de solo lectura. La casilla de verificación **Use GPS** está oculta en modelos de radio sin hardware GPS.
   - Desmarque **Use GPS** (o si la casilla de verificación no está presente) para ingresar un cuadrado de cuadrícula manualmente. El valor se guarda en `FreeDvMyGrid` y se convierte a mayúsculas al guardarlo.
3. Opcionalmente, complete **Station Msg:** con una breve nota de texto libre. Este mensaje aparece junto a su indicativo en el mapa público del reportador FreeDV y se guarda en `FreeDvMyMessage`.
