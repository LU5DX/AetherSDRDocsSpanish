...
AetherDSP Settings le ofrece un control detallado sobre los motores de reducción de ruido del lado del cliente en AetherSDR. Use este diálogo para ajustar el equilibrio entre la supresión de ruido y la fidelidad de voz en cuatro motores configurables: NR2, NR4, MNR y DFNR.
...
Abra el diálogo mediante `Settings > AetherDSP Settings...`. El diálogo contiene seis pestañas — **NR2**, **NR4**, **MNR**, **DFNR**, **RN2** y **BNR** — cada una dedicada a un motor de reducción de ruido diferente. La configuración se guarda de inmediato al cambiar cualquier control; no se requiere botón Apply ni OK.

### Pestaña NR2

NR2 es un motor de reducción de ruido musical en el dominio de la frecuencia. Sus parámetros controlan la agresividad con que se suprime el ruido y la forma en que el motor distingue la voz del ruido.
...
- **Gain Method** selecciona el mapeo de la curva de ganancia aplicado durante la reducción de ruido. Gamma coincide con los patrones típicos de amplitud de voz; Trained utiliza un modelo construido a partir de muestras reales de voz y ruido.
...
- **AE Filter (eliminación de artefactos)** activa o desactiva un postfiltro que reduce el timbre residual y los artefactos musicales comunes en el procesamiento en el dominio de la frecuencia.
...
- **Smoothing:** controla la rapidez con que la estimación de ruido sigue los cambios. Valores más altos producen una adaptación más estable pero más lenta.
...
NR4 utiliza la biblioteca libspecbleach para la reducción de ruido basada en sustracción espectral, con control independiente sobre la intensidad de supresión y el modelado espectral.
...
- **Noise Estimation Method** selecciona cómo NR4 estima el piso de ruido. SPP-MMSE equilibra la estimación de ruido con la preservación de la voz; Brandt utiliza suavizado recursivo en bandas críticas; Martin emplea mínimos espectrales continuos.
- **Adaptive Noise Estimation** habilita la reestimación continua del piso de ruido a medida que las condiciones cambian.
...
- **Enable MNR (macOS only)** activa o desactiva el motor. El estado inicial refleja lo que el motor de audio informa en el momento en que se abre el diálogo.
...
La pestaña BNR cubre la reducción de ruido de NVIDIA. La intensidad se controla desde el menú superpuesto, no desde este diálogo.
...
- En la pestaña NR4, dejar **Smoothing (%):** y **Whitening (%):** en 0 produce el sonido más natural; auméntelos solo si el ruido residual resulta molesto.
- Use **Reset Defaults** en la pestaña NR2 o NR4 para recuperar una línea de base conocida como válida antes de experimentar.
...
- [Habilitar o deshabilitar la estimación adaptativa de ruido en NR4](enable-or-disable-nr4-adaptive-noise-estimation.md)
- [Ajustar la profundidad de enmascaramiento y la intensidad de supresión en NR4](tune-nr4-masking-depth-and-suppression-strength.md)
- [Habilitar MNR en macOS y ajustar su intensidad](enable-mnr-on-macos-and-set-its-strength.md)
