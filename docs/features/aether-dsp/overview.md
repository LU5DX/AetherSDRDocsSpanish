...
Los ajustes de AetherDSP le brindan un control detallado sobre los motores de reducción de ruido del lado del cliente de AetherSDR. Use este diálogo para ajustar el equilibrio entre la supresión de ruido y la fidelidad de la voz en seis motores configurables: NR2, NR4, MNR, DFNR, RN2 y BNR.

## Antes de comenzar

- No se requiere conexión de radio para abrir o ajustar la Configuración de AetherDSP.
- Cada motor debe habilitarse por separado (desde el panel de applet o el menú superpuesto) antes de que sus ajustes surtan efecto.

## Cómo funciona

Abra el diálogo a través de `Settings > AetherDSP Settings...`. El diálogo contiene seis pestañas — **NR2**, **NR4**, **MNR**, **DFNR**, **RN2** y **BNR** — cada una cubre un motor de reducción de ruido diferente. Al hacer clic en una pestaña, también se activa o se omite ese motor; los seis interruptores DSP actúan como selectores exclusivos y controles de habilitación/deshabilitación del motor. Los ajustes se guardan inmediatamente cuando se cambia cualquier control; no se requiere ningún botón Aplicar o Aceptar.

El diálogo tiene una barra de título con degradado sin marco de 18 px con un glifo de agarre (⋮⋮) a la izquierda y botones de control de ventana (—, □, ×) a la derecha, que coincide con la familia de chrome de NetworkDiagnosticsDialog y AetherialAudioStrip. Los controles dentro del diálogo son proporcionados por un `AetherDspWidget` integrado en modo diálogo, con todas las fuentes escaladas a 13 px. La posición y el tamaño del diálogo se conservan entre sesiones usando la clave de ajuste `AetherDspDialogGeometry`. El fondo y los colores del diálogo siguen el tema actual a través de `ThemeManager`.

Cada botón de alternancia en la fila superior tiene un nombre de objeto de la forma `dspMethodBtnNR2`, `dspMethodBtnNR4`, etc., y un nombre accesible que incluye la etiqueta y "método de reducción de ruido". Esto permite que los puentes de automatización y las tecnologías de asistencia identifiquen cada control de motor específico.

Cuando selecciona una pestaña DSP, AetherSDR recuerda el último motor de reducción de ruido del lado del cliente usado en el ajuste `LastClientNr`. Si ese ajuste apunta a `DFNR` en una compilación sin soporte DeepFilterNet, se borra automáticamente.

### Pestaña NR2

NR2 es un motor de reducción de ruido musical en el dominio de la frecuencia. Sus parámetros controlan cuán agresivamente se suprime el ruido y cómo el motor identifica la voz frente al ruido.
...
- **Gain Method** selecciona la asignación de la curva de ganancia aplicada durante la reducción de ruido. Gamma coincide con los patrones típicos de amplitud del habla; Trained usa un modelo construido a partir de muestras reales de voz y ruido.
...
- **AE Filter (eliminación de artefactos)** activa un postfiltro que reduce el timbre y los artefactos musicales comunes en el procesamiento del dominio de la frecuencia.
...
NR4 utiliza la biblioteca libspecbleach para la reducción de ruido basada en sustracción espectral, con control independiente sobre la fuerza de supresión y la forma espectral. En Windows, NR4 requiere el conjunto de herramientas LLVM (clang-cl) para compilar los arreglos de longitud variable C99 de libspecbleach. Si AetherSDR se compiló sin LLVM, la alternancia NR4 está deshabilitada y una información sobre herramientas explica la dependencia faltante.
...
- **Noise Estimation:** selecciona el estimador del piso de ruido. MMSE equilibra la estimación de ruido con la preservación de la voz; Brandt utiliza suavizado recursivo en todas las bandas críticas; Martin utiliza mínimos espectrales móviles.
...
MNR es un motor de reducción de ruido MMSE-Wiener con suavizado de ganancia asimétrico. Está disponible solo en macOS; en las compilaciones de Windows y Linux, la alternancia MNR está atenuada porque el motor no tiene un backend en esas plataformas.
...
- **Enable MNR (solo macOS)** enciende o apaga el motor. El estado inicial refleja lo que informa el motor de audio en el momento en que se abre el diálogo.
- **Strength** establece la agresividad desde suave (0) hasta máxima (100). El valor se conserva como una cifra normalizada de 0.00–1.00.

### Pestaña DFNR

DFNR utiliza la red neuronal DeepFilterNet3 para la supresión profunda de ruido. Si AetherSDR se compiló sin soporte DeepFilterNet, la alternancia DFNR está deshabilitada y una información sobre herramientas explica la dependencia faltante.
...
- **Post-Filter Beta** aplica un filtro de postprocesamiento adicional para una supresión extra más allá de lo que proporciona la red neuronal.
...
La pestaña BNR cubre la reducción de ruido de NVIDIA. La intensidad se controla desde el menú superpuesto, no desde este diálogo. En las compilaciones sin el SDK de NVIDIA Broadcast, la alternancia BNR está atenuada.
...
Haga clic y arrastre cualquier borde o esquina del diálogo para redimensionarlo. El cursor cambia para indicar la dirección de redimensionamiento. Una zona de impacto de redimensionamiento de 6 px se extiende hacia adentro desde cada borde.
...
- [Configurar el beta del postfiltro DFNR para una supresión adicional](configure-dfnr-post-filter-beta-for-extra-suppression.md)
