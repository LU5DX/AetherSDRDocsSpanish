...
Esta página explica cómo ajustar la profundidad máxima de reducción de ruido y el umbral de presencia de voz de NR2 en AetherSDR. Modificar estos dos controles deslizantes permite equilibrar la agresividad con la que NR2 suprime el ruido y la fidelidad con la que preserva la voz.

## Antes de comenzar

- AetherSDR debe estar en ejecución. No se requiere conexión a un radio para cambiar estos ajustes.
- NR2 debe estar ya activo en un slice receptor para que los cambios surtan efecto audible de inmediato.
...
| **Smoothing:** | `0.85` | `0.50`–`0.98` | `NR2GainSmooth` | Controla la rapidez con la que la estimación de ruido sigue los cambios. Valores más altos producen una estimación más estable, pero una adaptación más lenta a cambios en el ruido. |
| **AE Filter (artifact elimination)** | Activado | Activado / Desactivado | `NR2AeFilter` | Activa o desactiva el postfiltro antiartifactos que reduce el ruido musical y el repique. |
...
- Reducir **Voice Threshold:** por debajo de `0.15` puede provocar que el ruido residual se filtre durante las pausas en la voz, porque una mayor parte de la señal se clasifica como voz. Auméntelo si observa este comportamiento.
- Si la estimación de ruido reacciona demasiado lentamente ante ruido en ráfagas, reduzca **Smoothing:** hacia `0.60`. Si el gate de ruido suena entrecortado, auméntelo hacia `0.95`.
- Se recomienda mantener **AE Filter (artifact elimination)** habilitado en la mayoría de las condiciones; deshabilítelo únicamente si nota que el propio postfiltro introduce artefactos.

## Solución de problemas

- **La voz suena hueca o sobreprocesada** — **Reduction Depth:** es demasiado alto o **Voice Threshold:** es demasiado alto. Reduzca **Reduction Depth:** y/o reduzca **Voice Threshold:** para que se preserven más componentes de voz.
- **El ruido sigue siendo audible durante las pausas en la voz** — **Voice Threshold:** es demasiado bajo, lo que provoca que las pausas se clasifiquen como voz. Aumente **Voice Threshold:** hacia `0.30`–`0.40`.
