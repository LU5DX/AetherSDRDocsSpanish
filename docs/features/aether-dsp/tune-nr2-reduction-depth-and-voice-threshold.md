...
| **AE Filter (artifact elimination)** | On | On / Off | `NR2AeFilter` | Activa o desactiva el postfiltro antirruido. |
...
| **Post-Filter Beta** | 0.00 | 0.00–0.30 | `DfnrPostFilterBeta` | Aplica un postfiltro adicional para mayor supresión. El control deslizante almacena el valor*100 internamente. |

## Consejos

- Para operación de voz en SSB con NR2, comience con **Reduction:** en `1.50` y **Threshold:** en `0.20`. Si la voz suena recortada o hueca, reduzca **Reduction:** hacia `1.00`.
- Reducir **Threshold:** por debajo de `0.15` puede hacer que el ruido residual se filtre durante las pausas del habla porque más señal se clasifica como voz. Auméntelo si nota esto.
- Si la estimación de ruido reacciona demasiado lentamente al ruido de ráfaga, reduzca **Smoothing:** hacia `0.60`. Si la compuerta de ruido suena entrecortada, auméntelo hacia `0.95`.
- Se recomienda mantener **AE Filter (artifact elimination)** activado para la mayoría de las condiciones; desactívelo solo si nota que el postfiltro introduce artefactos.
...
- **El ruido aún es audible durante las pausas del habla** — **Threshold:** es demasiado bajo, lo que hace que las pausas se clasifiquen como voz. Aumente **Threshold:** hacia `0.30`–`0.40`.
