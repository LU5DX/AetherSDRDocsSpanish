...
La configuración de AetherDSP es un cuadro de diálogo que le permite ajustar los motores de reducción de ruido del lado del cliente disponibles en AetherSDR: NR2, NR4, MNR, DFNR, RN2 y BNR. Cada motor tiene su propia pestaña. Abra el diálogo desde `Settings > AetherDSP Settings...`.

El diálogo utiliza una barra de título personalizada con degradado, botones de control de ventana, redimensionamiento en 8 ejes y capacidad de arrastrar para mover. El diálogo aloja un `AetherDspWidget` integrado que contiene todos los controles. Las conexiones de señal existentes al diálogo continúan funcionando sin cambios.
...
`NR2GainMethod` se almacena como un entero de 0 a 3 (Linear=0, Log=1, Gamma=2, Trained=3). `NR2NpeMethod` se almacena como un entero de 0 a 2 (OSMS=0, MMSE=1, NSTAT=2). `NR2GainMax` se almacena internamente como el valor × 100.
...
`NR4NoiseEstimationMethod` se almacena como un entero de 0 a 2 (MMSE=0, Brandt=1, Martin=2). `NR4ReductionAmount` se almacena internamente como el valor × 10.
...
MNR es el motor de reducción de ruido MMSE-Wiener de AetherSDR, disponible solo en macOS. La activación de MNR aparece atenuada en las compilaciones de Windows y Linux.

### Antes de comenzar

- AetherSDR debe ejecutarse en macOS. La casilla de verificación **Enable MNR (macOS only)** solo está presente en las compilaciones de macOS.
...
| Control | Tipo | Valor predeterminado | Rango válido | Clave de configuración |
|---|---|---|---|---|
| Enable MNR (macOS only) | Casilla de verificación | *(se lee del motor de audio al inicio)* | Activado / Desactivado | `MnrEnabled` |
...
- Debido a que el estado inicial activado de MNR se lee en vivo del motor de audio al abrir el diálogo, la casilla de verificación refleja el estado actual del motor en lugar de un valor predeterminado guardado fijo.
...
La pestaña **BNR** muestra información sobre el motor de reducción de ruido de NVIDIA. La intensidad se controla desde el menú superpuesto, no desde este diálogo. La activación de BNR aparece atenuada en compilaciones sin el SDK de NVIDIA Broadcast.
...
4. Arrastre **Post-Filter Beta** para aplicar supresión adicional de posfiltrado. Déjelo en 0.00 si no se necesita supresión adicional.
...
`DfnrPostFilterBeta` se almacena internamente como el valor × 100.
...
- Haga clic y arrastre cualquier borde o esquina del diálogo para redimensionarlo. El cursor cambia para indicar la dirección del redimensionamiento.
- La zona de impacto para redimensionar se extiende 6 px desde los bordes del widget de contenido interno.
