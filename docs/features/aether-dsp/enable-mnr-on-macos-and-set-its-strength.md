...
Configuración de AetherDSP es un diálogo que le permite ajustar los motores de reducción de ruido del lado del cliente disponibles en AetherSDR: NR2, NR4, MNR, DFNR, RN2 y BNR. Cada motor tiene su propia pestaña. Abra el diálogo desde `Settings > AetherDSP Settings...`.

El diálogo utiliza una barra de título con degradado personalizado, botones de control de ventana, redimensionamiento de 8 ejes y funcionalidad de arrastrar para mover. El diálogo alberga un `AetherDspWidget` incrustado que contiene todos los controles. Las conexiones de señal existentes al diálogo continúan funcionando sin cambios.
...
`NR2GainMethod` se almacena como un entero de 0 a 3 (Linear=0, Log=1, Gamma=2, Trained=3). `NR2NpeMethod` se almacena como un entero de 0 a 2 (OSMS=0, MMSE=1, NSTAT=2). `NR2GainMax` se almacena internamente como valor × 100.

### Consejos

- El método de ganancia **Gamma** es adecuado para la mayoría de las voces en SSB. **Trained** puede mejorar los resultados en señales débiles.
...
`NR4NoiseEstimationMethod` se almacena como un entero de 0 a 2 (MMSE=0, Brandt=1, Martin=2). `NR4ReductionAmount` se almacena internamente como valor × 10.
...
- Si la pestaña NR4 está atenuada en Windows, instale LLVM y reconstruya para habilitar el motor.

---

## Pestaña MNR

MNR es el motor de reducción de ruido MMSE-Wiener de AetherSDR, disponible solo en macOS. La opción de activación MNR está atenuada en las compilaciones para Windows y Linux.

### Antes de comenzar

- AetherSDR debe ejecutarse en macOS. La casilla de verificación **Enable MNR (macOS only)** solo está presente en las compilaciones para macOS.
...
4. Arrastre el control deslizante **Strength** para ajustar la agresividad. 0 es la configuración más suave; 100 es la máxima reducción de ruido.
...
- Debido a que el estado inicial activado de MNR se lee directamente del motor de audio al abrir el diálogo, la casilla de verificación refleja el estado actual del motor en lugar de un valor predeterminado guardado fijo.
...
La pestaña **BNR** muestra información sobre el motor de reducción de ruido de NVIDIA. La intensidad se controla desde el menú superpuesto, no desde este diálogo. La opción de activación BNR está atenuada en compilaciones sin el SDK de NVIDIA Broadcast.
...
El diálogo Configuración de AetherDSP utiliza una barra de título personalizada sin marco.

### Barra de título

- 18 px de altura con un fondo degradado diseñado por el tema actual.
- Un glifo de agarre (⋮⋮) a la izquierda y "AetherDSP Settings" en texto en negrita de 10 px.
...
- La zona de impacto de redimensionamiento se extiende 6 px desde los bordes del widget de contenido interno.
