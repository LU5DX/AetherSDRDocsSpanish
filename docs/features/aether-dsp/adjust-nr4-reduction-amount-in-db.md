...
El cuadro de diálogo **Configuración de AetherDSP** (`Settings > AetherDSP Settings...`) controla los motores de reducción de ruido del lado del cliente integrados en AetherSDR: NR2, NR4, MNR, RN2, BNR y DFNR. Puede abrirlo en cualquier momento; no se requiere una conexión de radio para cambiar estos parámetros. Todos los valores se guardan automáticamente al cerrar el cuadro de diálogo. Los seis módulos DSP se seleccionan mediante una fila de botones de alternancia en la parte superior; al hacer clic en un botón también se activa o desvía ese motor.

El cuadro de diálogo utiliza una barra de título personalizada sin marco, con controles de ventana que siguen la familia cromática de NetworkDiagnosticsDialog y AetherialAudioStrip. La posición y el tamaño del cuadro de diálogo se conservan automáticamente entre sesiones. Arrastre la barra de título para mover el cuadro de diálogo; haga doble clic en la barra de título para alternar entre maximizar y restaurar. Arrastre cualquier borde o esquina para cambiar el tamaño (zona de cambio de tamaño de 6 píxeles alrededor del widget de contenido interno).
...
**AE Filter (artifact elimination)** activa o desactiva el filtro posterior contra artefactos (`NR2AeFilter`). Manténgalo habilitado a menos que esté probando específicamente la salida NR2 sin procesar.
...
**Threshold:** (`NR2Qspp`) establece el umbral de probabilidad de presencia de voz por debajo del cual NR2 trata el audio como ruido. Auméntelo si se suprime el habla; disminúyalo si el ruido se cuela durante las pausas.
...
NR4 utiliza la librería libspecbleach. En Windows, NR4 requiere LLVM (clang-cl) para compilar; si LLVM no está instalado, el botón de alternancia de NR4 se atenúa con un mensaje emergente que indica que debe instalar LLVM desde llvm.org y recompilar. Haga clic en la pestaña **NR4** para acceder a sus controles.
...
**Reduction (dB):** (`NR4ReductionAmount`) establece la reducción máxima de ruido que NR4 aplica en decibelios. En 0.0 dB, NR4 no aplica atenuación; en 40.0 dB, aplica la supresión máxima disponible. El valor predeterminado de 10.0 dB es adecuado para la mayoría de las condiciones de SSB.
...
- **Smoothing (%)** y **Suppression** interactúan con la cantidad de reducción. Si aumentar `NR4ReductionAmount` causa ruido musical, intente aumentar el suavizado primero.
...
MNR es el motor de reducción de ruido MMSE-Wiener con suavizado asimétrico de ganancia. Está disponible solo en macOS. En las compilaciones de Windows y Linux, el botón de alternancia de MNR se atenúa; el motor no tiene un backend en esas plataformas. Haga clic en la pestaña **MNR** para acceder a sus controles.

### Controles

| Control                              | Tipo                                                                                                        | Valor predeterminado                                                                                          |
|--------------------------------------|-------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------|
| **Enable MNR (macOS only)**          | Casilla de verificación                                                                                     | Se lee desde el motor de audio                                                                                |
| **Strength**                         | Deslizador                                                                                                  | 100                                                                                                           |

### Descripciones de los controles

**Enable MNR (macOS only)** (`MnrEnabled`) habilita el motor de reducción de ruido MMSE-Wiener con suavizado asimétrico de ganancia. El estado inicial de la casilla de verificación refleja el estado actual del motor de audio en el momento en que se abre el cuadro de diálogo.
...
La pestaña **BNR** muestra información sobre el motor de reducción de ruido de NVIDIA. La intensidad de BNR se controla desde el menú superpuesto, no desde este cuadro de diálogo. En las compilaciones sin el SDK de NVIDIA Broadcast, el botón de alternancia de BNR se atenúa.
...
**Post-Filter Beta** (`DfnrPostFilterBeta`) aplica un filtro posterior adicional para una supresión extra más allá de la que DeepFilterNet3 proporciona por sí solo. En 0.00, el filtro posterior está inactivo. El valor se almacena internamente multiplicado por 100.
...
- **La voz suena hueca o distorsionada con valores altos de reducción NR4**: reduzca **Reduction (dB)** y active **Adaptive Noise Estimation** para que la estimación del piso de ruido se mantenga precisa.
